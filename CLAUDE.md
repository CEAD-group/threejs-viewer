...
- `22_polyline_picking.py` - Pick a point along a polyline from the browser back to Python. Three pickable lines of increasing density share the scene (the screen-nearest to the cursor wins): a **square** from 4 corner points (widely-spaced nodes prove picking is *continuous* — the marker glides through corners without snapping; corners at 0.25/0.50/0.75/1.0 of arc length), a **mixed-detail** line alternating long sparse straight runs with dense coils + a ripple, and a dense self-overlapping **spiral vase** (the ball-of-yarn toolpath from example 20, ~12k points; `D`/`Shift+D` depth cues help read it). Each coloured by its own arc-length fraction (turbo). `on_polyline_pick(callback)`; hover glides a marker to the closest point and shows the fraction, a click sends `{id, fraction, point, ...}` to Python where the callback prints which line was hit and drops a red sphere on it (browser → Python → browser round trip).
