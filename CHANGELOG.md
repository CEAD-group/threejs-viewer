...
- **`strand_collapse=True` flag on `add_parametric_tube` removes self-intersecting strand folds at tight inside corners.** Miter framing fixes the *outside* of a sharp corner but not the inside: when the local curvature κ exceeds 1/half-width, the inner offset surface self-intersects geometrically — adjacent rings' inner-side vertices loop past each other and the renderer paints a chaotic triangle fan that no frame trick can repair. The new flag runs a post-process per "strand" (the per-cross-section-vertex polyline running along the tube — 6 of them on the chamfered hex section): for every ring index *i*, scan the segments at ±50 rings (configurable in source as `TUBE_STRAND_COLLAPSE_WIN`) and test the perpendicular foot of `p_i` onto each segment `(p_k, p_{k+1})`. When the foot lands inside the segment with distance below 5% of the bead's largest cross-section dimension, the run `[min(i,k) .. max(i,k+1)]` is flagged AND the closest-pair midpoint between segment `(p_a, p_{a+1})` and segment `(p_k, p_{k+1})` (where `a = i` for forward detection, `a = i-1` for backward) is recorded as the per-detection fold target. The segment-segment closest-pair midpoint sits in the *gap* between the two folding segments — exactly on the perpendicular bisector of the closest pair of points on the two segments, which approaches the strand's actual self-intersection apex as the gap shrinks to zero. Overlapping detections merge; each merged range collapses to the midpoint of its **tightest** constituent detection (smallest closest-pair gap²). Picking the tightest detection rather than averaging across all of them keeps the snap target at the actual crossing apex regardless of how many looser long-range detections fired in the same window — averaging mixed in midpoints biased toward the medial axis of the fold's interior, dragging the snap point inboard of the crossing. Coincident vertices produce zero-area quads (skipped in raster), so the visible result at the fold is a clean hard crease that sits at the actual fold location while the rest of the tube keeps its width. Both forward and backward `k` are scanned per `i` because point-to-segment is asymmetric — `p_i → seg(k,k+1)` is not equivalent to `p_k → seg(i,i+1)` — so a forward-only scan would miss configurations where a later vertex is close to an earlier segment but not vice versa. The scan walks every `k` in the window per direction (no early-exit) and per `i` per direction synthesises one detection that carries the largest-`k` hit (for range coverage) AND the tightest-gap midpoint (for the snap target) — these can come from different `k` values, and decoupling them is the whole point. First-hit-from-furthest early-exit was dropped because it systematically picked the loose pair at gap ≈ tol; for deep folds (the widest strand at a sharp inside corner) the tightest pair sits at smaller `k`, which the early-exit scan never visited, so the snap landed on the medial-axis loop centre instead of the actual self-intersection. Cost is O(N · W · nCs). Off by default — opt in per call: `v.add_parametric_tube("bead", spine, widths, heights, strand_collapse=True)`. Endpoints are protected (collapse range clamped to `[1, nSpine-2]`) so cap rings stay anchored to the spine frame. Showcased in `examples/21_tube_corner_zfight.py` (baseline left, `strand_collapse=True` right, same pathological Catmull-Rom spine). Microbenchmark in `bench/strand_collapse_bench.mjs` compares the shipped algorithm against four discarded variants.
