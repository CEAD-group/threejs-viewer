...
- **`parametric_tube` primitive** — variable-cross-section extruded tube built on the client from per-spine-point parameter arrays (spine + widths + heights + optional orientations + optional colors). Rounded-rectangle cross-section with configurable corner radius and vertex count. Supports `draw_range` (snapped to whole ring-pairs so the cut edge stays clean) and the `draw_ranges` animation channel. Wire transfer is O(N) instead of O(N × nCs), ~6× smaller than a baked mesh. See `examples/18_parametric_tube.py`.
