...
- **The EDL composer now ends with an SMAA pass**, so line and mesh edges under `Shift+D` / `set_edl(True)` are anti-aliased again. The direct render gets hardware MSAA, but composer targets are single-sampled and a multisampled target cannot expose the depth texture the EDL pass reads, so EDL-on rendering had jagged edges. `SMAAPass` runs after `OutputPass` (its luma edge detection wants display-referred colour) with its blend material set to `NoBlending`, for the same transparent-background reason as `OutputPass`. Fixed cost: ~1 ms at 1080p, ~4 ms at 4K, independent of scene size.
- The composer normalises its buffer parity after each render and `dispose()` clears the SMAA pass reference alongside the others.

### GLTF watcher example and material sampler asset (#180, #181)

- **`examples/31_watch_gltf.py`** (contributed by DAlexanderr): live-reload one or more `.gltf` files while editing them in Blender or a text editor. Each argument is a `.gltf` path or a directory `name` meaning `name/name.gltf`; each source is polled for mtime changes, converted to `.glb` with `gltf-transform` (global install or `npx @gltf-transform/cli`), stage-loaded under a fresh revision id, and only after `wait_for_assets` lands is that source's previous revision deleted — the scene never goes blank and unrelated objects are left alone. A failed conversion is retried on the next poll.
- **`examples/materials/materials.gltf`** is the demo input: a Blender PBR material sampler (24 materials: clearcoat / sheen / anisotropy / specular / IOR) whose geometry is generated by `examples/materials/make_materials_gltf.py`. One upright "pac-man" cylinder per material, flat on the floor in the file's Z-up frame, a 90° wedge removed, the top rim filleted on one half and 45°-chamfered on the other so both edge treatments show on each material, at the original export's grid positions. Each body's **material name lies on the floor in front of it** in a built-in 5x7 block font (glyph meshes instanced per letter, so the 30 bodies share one body geometry and the `.bin` is ~115 KB). Deterministic output; the material definitions are carried over from the export unchanged.

### Browser tests: explicit synchronization (#176)

- **Fixed sleeps in the Playwright suite are replaced by two barriers**: `settle(client)` — a `query_scene` round-trip on the ordered WebSocket (proving every earlier message was handled) followed by draining `pending_fetches` (proving binary payloads have landed in the scene graph) — and `frames(page)` for anything driven by the render loop. A `page.evaluate` travels over CDP, which has no ordering against the WebSocket, so a sleep could only make that race less likely, never impossible; two such flakes cost the 0.0.49 release a re-run. 30 sites converted; the sleeps that remain are genuinely wall-clock waits (octree-LOD node streaming, the 2 Hz tube-LOD throttle, rAF-coalesced resize/hover, the superseded-fetch grace). The fixture's WS-connect budget goes 30 s → 120 s.
...
- **Zero-length spine segments no longer snap the frame tangent to +X (#117).** `computeMiterFrames` seeds from the first non-degenerate segment and carries the previous direction through a degenerate (exactly-coincident) one, instead of falling back to a hardcoded `(1,0,0)`. A duplicate vertex at a bend — e.g. a sharp width-step twin ring — now renders as a clean step rather than a fold-fan shard. Shared by the main-thread build and the LOD worker.
- **The LOD-dispatch skip gate uses camera distance, not model size (#118).** The gate now references the camera's actual distance to the tube's bounding-sphere **surface** (radius scaled by the object's max world-axis scale; floored at 1% of the radius, capped by the old 2×radius proxy; no per-frame allocation). Close-in zooming moves the camera a few cm while the LOD epsilon (∝ camera distance) shrinks an order of magnitude — the old model-size gate never re-fired there, so a stale coarse tube rendered up close with visible RDP faceting; the surface-distance gate re-triggers the worker on small close-in moves while behaving as before far away.
- **`strand_collapse`: large-segment exemption + lower default snap factor (#119).** A ring whose shorter adjacent spine segment is ≥ `large_seg_factor` bead-widths (default 1.0, `0` = off, tunable via the `strand_collapse` dict) is treated as an open straight and never snapped — confining collapse to genuinely dense wipe-loop regions and removing false snaps triggered by degenerate tangents at nearby micro-segments; the exemption is inert on genuine dense folds and only ever *skips* a snap. **Default `max_snap_factor` drops 1.0 → 0.5**: the full-bead reach over-snaps tight/messy folds (spiky-cusp fin + z-fighting seam, over-snap spikes along the adjacent straight on jittered reversals), while 0.25 under-shoots real wide-bead corners (strands stop short of the apex → protruding wedge); 0.5 reaches the apex identically to 1.0 on real corners (0.00% pixel diff vs the tuned #50 bulb baseline) and measures at its quality plateau (identical to 1.0 on the bench's messy-zigzag scene: intersecting-edge pixels cut to 76% of collapse-off at footprint IoU 0.998, unchanged triangles). Also fixes a latent bug where the non-LOD tube path dropped `largeSegFactor` from the reconstructed collapse config, so the knob never reached the pass.

### Parametric tube: split huge tubes across the WebGL per-draw index cap (#113, #114)

- **A tube whose index buffer exceeds the WebGL per-draw limit no longer silently loses its tail.** Past `webgl.max-vert-ids-per-draw` (30M on Firefox/ANGLE; driver-dependent, not reliably queryable) the GPU truncates the draw with only a console warning — a 1.24M-point spine is 44.6M indices, and `lod=true` only masked the zoomed-out view (a close zoom refined back over the cap, #114). `applyTubeDrawCap` splits any tube over a conservative `MAX_TUBE_INDICES_PER_DRAW = 24M` into `geometry.addGroup` chunks (whole-triangle-aligned, each ≤ cap) plus a 1-element material array, so three.js issues one `drawElements` per group. Groups partition the index buffer positionally and the renderer intersects `geometry.drawRange` with each group, so the single `setDrawRange(0, N)` reveal and the endCap relocation span chunks with no special handling; the M/N debug material swap is group-aware. Applied on both the non-LOD build and every LOD-worker geometry swap. Tubes under the cap keep a single material and no groups — byte-identical to before. Test seam: `window.__tubeDrawCapOverride`.

### Tube-corner quality bench (`bench/tube_lab/`) (#122)

- **Dev tooling** (not shipped, not in CI): the tube-corner investigation harness is now committed so its numbers stay reproducible bit-for-bit. Scores spine transforms × `strand_collapse` settings on three WebGL-readback measures — intersecting-line dark pixels in the wireframe overlay, solid-footprint IoU vs the collapse-off baseline, triangle count — across corner-archetype scenes (messy decimated+jittered zigzag / 90° sharpV guard / tight hairpin / mixed sharp-corner-plus-dense-curve), ending in a strict per-scene success gate (`IoU ≥ 0.99 ∧ tris ≤ off ∧ darkPx ≤ off`) that a candidate approach must clear on every scene before promotion to `viewer.js` is worth discussing. Includes the curvature-adaptive resampler prototype `adaptive(step, pin_deg, dense_factor)` (pins sharp vertices, rounds only dense spans; passes 3/4 scenes, byte-identical to off on sharpV) and `FINDINGS.md`, the consolidated investigation history (verified detector constants, tuning chronology, rejected approaches). Run: `uv run python bench/tube_lab/zr_metric.py`.
...
### Accelerated polyline/tube picking on huge spines (#112)
...
### Playback & frontier sync

- **Nozzle-vs-frontier desync on travel-split toolpaths — fixed (the real-file bug behind the #97 report's screenshot).** Producers send `draw_range` values in the convention `value = fractional_point_index / (n-1)`, but `register_toolpath_group` built its per-segment ranges with `/n` — recovering the frontier's local index as `value·n − s` instead of `value·(n−1) − s`, a systematic lead of exactly `value` points. One point is one G-code segment, so the error was invisible in dense clusters but **hundreds of mm on long raster moves** near the end of a path. Measured on the reporting 497,818-point NC program at the reported playhead: worst frontier-vs-true-tip error **531 mm → 3.8 mm** (residual = float32 channel quantization, sub-bead-width). `travelEndFracs` (travel-line reveal) switched to the same `/(n−1)` convention. Only group (travel-split) toolpaths were affected — single-tube paths always used `value·(n−1)` directly.
- **Tube-LOD frontier placement under RDP collapse — fixed.** `remapDrawRangeToReducedPairs` interpolated across a kept span **by index**; RDP collapses collinear runs regardless of point spacing, so one long segment followed by a dense flatten-tolerance cluster put the frontier almost anywhere on the span (measured 98 mm error on a 106 mm run). The remap now takes the true frontier position from the retained original spine at the fractional index and projects it onto the reduced chord — placement error is bounded by the same epsilon the reduced geometry is drawn at.
