...
- **Read-side messages (`set_color`, `set_opacity`, `set_visibility`, `update_transform`, `set_draw_range`, `update_polyline_colors`, `update_parametric_tube_colors`) arriving during a binary load are no longer silently dropped.** The four binary loaders (`add_model_binary`, `add_polyline_binary`, `add_mesh_binary`, `add_parametric_tube_binary`) each spawn a fire-and-forget `(async () => {...})()` IIFE that does `fetch` + geometry build + `_objects.set(id, obj)`. The WebSocket dispatcher fell through the case's `break` immediately, so any subsequent message targeting the same id was processed *before* the IIFE registered the object — `this._objects.get(id)` missed and the handler silently no-opped. Whether `set_color` "worked" was undefined behaviour: cold HTTP roundtrips lost the race (silent drop), warm caches or pre-existing objects won it (applied). This had been masquerading as "GLB native materials" in cold-load paths because `set_color` traversal over a missing target was unobservable. Fix: introduce per-id deferred tracking (`this._inflightLoads`). Each binary-loader case branch synchronously installs a `_makeDeferred()` keyed by id before spawning its IIFE; the IIFE resolves on successful registration and rejects on error, stale token (load superseded by a newer add with the same id), or mid-load `_deleteObject`. A new `_withObject(id, opName, fn)` helper applies `fn` synchronously if the object is registered, defers onto the in-flight promise otherwise, or silently no-ops if neither (matches today's behaviour for genuinely missing ids). On load failure / cancellation, queued ops drop with a single `console.warn` naming the op and id. The seven read-side cases route through `_withObject` at the case-branch level only — the private helpers (`_setOpacity`, `_setVisibility`, `_setDrawRange`, `_updateTransform`) keep their "obj exists or skip" semantics so animation appliers and `_batchUpdate` aren't impacted. Parallel binary fetches are preserved (no IIFE-level `await` in the dispatcher). Ordering between queued ops is FIFO via microtask scheduling — two `set_color` calls during a load apply in the order they arrived. `_deleteObject` rejects any in-flight load for the id so queued ops on a deleted target drop cleanly; binary loaders pass `preserveInflight: true` through `_addObject` → `_deleteObject` so their own pre-registration cleanup doesn't reject the deferred they just installed. Regression covered by `tests/test_browser.py::test_set_color_during_binary_load_is_honoured`, `::test_set_visibility_during_binary_load_is_honoured`, and `::test_delete_during_binary_load_drops_queued_ops`.
