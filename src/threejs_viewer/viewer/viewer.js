...
//
// `outFrames` and `outScales` must be pre-allocated: Float32Array(nSpine*6)
// and Float32Array(nSpine).
/**
...
function buildGeometry(spine, widths, heights, upVec, ringColors, heightOffset) {
    ...
    computeMiterFrames(spine, nSpine, localFrames, miterScales,
    ...
    // Revolution caps. Endpoints have miter_scale=1 by construction, so this
    // pass doesn't need miter scaling (caps are at spine indices 0 and nSpine-1).
    ...
    return { positions, normals, colors, indices, localFrames, miterScales, capAngles, endCapPattern,
...
self.onmessage = function(e) {
    ...
    self.postMessage({
        ...
        localFrames: geo.localFrames, miterScales: geo.miterScales,
...
function buildParametricTubeGeometry(
    ...
    return {
        ...
        localFrames, miterScales, capAngles,
...
class ParametricTube {
    ...
    morphFrontierRing(fracRingPairs) {
        ...
        md.morphedState = { sx, sy, sz, w, h, ux, uy, uz, vx, vy, vz, miter };
    ...
    updateEndCap(lastVisibleRing) {
        ...
        let sx, sy, sz, w, h, ux, uy, uz, vx, vy, vz, miter;
        if (md.morphedState) {
                    ...
                    case 'add_parametric_tube_binary': {
                        ...
                        (async () => {
                            try {
                                ...
                                const { geometry, ringPairs, indicesPerRingPair, localFrames, miterScales, capAngles, capIndicesPerCap, endCapBase, endCapPattern } = buildParametricTubeGeometry(
