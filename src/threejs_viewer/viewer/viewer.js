...
// ---- Non-finite matrix guard (issue #162) ----
// A single NaN/Inf element anywhere in a 4x4 poisons `matrixWorld` for the
// whole subtree, every vertex lands at NaN clip space and three.js drops the
// object outright — mid-playback a link just vanishes, with no diagnostic.
// Every matrix write coming off the wire (animation channels + streaming
// transform updates) therefore goes through these helpers: a non-finite
// matrix is dropped and the object simply holds its last good pose.
//
// Cost: 16 `Number.isFinite` calls per *applied* matrix (not per element of
// every frame — only the one keyframe pair actually being applied this
// frame). Measured at ~6.5 us/frame for the 520-object stress example
// (examples/10_animation_stress_test.py), i.e. ~0.04% of a 16.7 ms frame
// budget, and an order of magnitude under the decompose/slerp/compose the
// linear path already runs per object.
/**
 * @param {ArrayLike<number>} src
 * @param {number} base
 */
function matrixElementsFinite(src, base) {
    for (let k = 0; k < 16; k++) {
        if (!Number.isFinite(src[base + k])) return false;
    }
    return true;
}

/**
 * Finite check for a loose transform component array (position/rotation/
 * quaternion/scale). Non-numeric entries are ignored — `Euler.fromArray`
 * takes an order string as its 4th element.
 * @param {ArrayLike<any>} arr
 */
function arrayFinite(arr) {
...
// One warning per object id — a bad keyframe is usually a whole bad channel,
// and warning per frame would flood the console at 60fps.
/**
 * @param {any} obj
 * @param {string} what
 */
function warnNonFiniteMatrix(obj, what) {
    if (obj.userData.__warnedNonFiniteMatrix) return;
    obj.userData.__warnedNonFiniteMatrix = true;
    const id = obj.userData.id || obj.name || '(unnamed)';
    console.warn(
        `${what}: '${id}' received a non-finite (NaN/Inf) matrix — holding the ` +
        `last good pose. Further non-finite matrices for this object are not reported.`);
}

/**
 * Copy a flat 4x4 at `src[base..base+16)` into `obj.matrix`, unless it holds a
 * non-finite element (then the previous pose is kept). Returns true if applied.
 * @param {any} obj
 * @param {ArrayLike<number>} src
 * @param {number} base
 * @param {string} what
 */
function setObjectMatrixChecked(obj, src, base, what) {
    if (!matrixElementsFinite(src, base)) {
        warnNonFiniteMatrix(obj, what);
        return false;
    }
    obj.matrix.fromArray(src, base);
...
/**
 * Same, for an already-composed Matrix4 (the interpolated result). NaN/Inf in
 * either endpoint propagates through lerp/slerp/compose, so checking the
 * output alone catches a bad keyframe on either side.
 * @param {any} obj
 * @param {THREE.Matrix4} mat
 * @param {string} what
 */
function setObjectMatrix4Checked(obj, mat, what) {
    if (!matrixElementsFinite(mat.elements, 0)) {
        warnNonFiniteMatrix(obj, what);
        return false;
    }
    obj.matrix.copy(mat);
...
// ========== Embedded-clip (GLTF/GLB) mixer helpers ==========
// A freshly-loaded model's AnimationMixer must NOT deform the model
// (issue #135): the old eager `action.play()` + `mixer.setTime(0)`
// statically bound every node to its t=0 keyframe (e.g. a track bellows
// compressed to scale 0.059) with nothing driving the clip. Instead the
// actions are created but not played, so the authored bind pose (the node
// transforms as exported) holds until the first clip-drive message —
// set_clip_time, set_clip_progress, or a clip_times animation channel —
// activates them. After that first drive, behavior is identical to before.

/**
 * @param {any} mixer THREE.AnimationMixer for a freshly-loaded model
 * @param {any[]} clips THREE.AnimationClip[]
 */
function prepareModelMixer(mixer, clips) {
    mixer.__tjsvActions = clips.map((/** @type {any} */ clip) => mixer.clipAction(clip));
    mixer.__tjsvDriven = false;
}

/** Start the deferred actions on the first drive (no-op afterwards). @param {any} mixer */
function activateMixerActions(mixer) {
    if (mixer.__tjsvDriven || !mixer.__tjsvActions) return;
    for (const action of mixer.__tjsvActions) action.play();
 ...
 * @param {any} mixer @returns {number[]} */
function mixerClipDurations(mixer) {
    return (mixer.__tjsvActions || []).map(
        (/** @type {any} */ action) => action.getClip().duration
    );
}

// ========== Declarative clip-time binding ==========
// bind_clip (issue #225) replaces a per-tick `set_clip_time` push with a
// standing declaration: `id`'s mixer time is a function of `source.id`'s own
// live LOCAL transform, re-resolved every render frame in `_animate()`
// instead of pushed over the WS. A push is subject to the same latest-wins
// dispatch coalescing as every other WS message (AGENTS.md), so a fast jog
// or drag can produce transform updates faster than they're dispatched and
// intermediate set_clip_time values get dropped — the bound clip then visibly
// skips instead of tracking the drag continuously. Resolving from state the
// renderer already holds every frame (the same live transform a drag or a
// streamed update already writes to `source.id`) needs no per-frame message
// at all, so it can't be coalesced away.

/** Read one named local-space channel off an Object3D's own transform (the
 * same values a drag or a streamed transform message already writes to).
 * @param {any} obj @param {string} channel `"translation"|"rotation"|"scale".x|y|z`
 * @returns {number|null} null for an unrecognised channel */
function readLocalChannel(obj, channel) {
            ...
            if (shouldInterpChannel(ch, baseNext, t)) {
                for (let i = 0; i < nObj; i++) {
                    ...
                    if (obj) {
                        ...
                        setObjectMatrix4Checked(obj, _lerpMatOut, 'transforms channel');
                    }
                }
            } else {
                for (let i = 0; i < nObj; i++) {
                    const obj = refs[i];
                    if (obj) {
                        setObjectMatrixChecked(obj, ch.data, base + i * 16, 'transforms channel');
                    }
                }
            }
        },
 ...
 * @param {AbortSignal} [signal] - cancels the fetch when the object is
 *   deleted or superseded, so a re-push never requests a dropped blob (#157)
 * @returns {Promise<ArrayBuffer>}
 */
async function fetchArrayBuffer(url, what, signal) {
...
// A blob fetch that fails because its object was deleted or re-pushed is
// expected fallout, not a defect (issue #157). The viewer aborts such fetches
// on delete/supersede, but the failure and the WebSocket message that explains
// it travel on different transports, so a 404 can beat its own explanation
...
// a Symbol is not equal to any string an object id could ever be — so this
// cannot collide with a real id no matter what a producer sends, and needs no
// validation on the id path to stay that way.
const ANIMATION_FETCH_KEY = Symbol('animation-blob-fetch');

/** Grace window in ms, with a test seam (`window.__supersededFetchGraceMs`). */
function supersededFetchGraceMs() {
    const override = typeof window !== 'undefined'
        ? /** @type {any} */ (window).__supersededFetchGraceMs
        : undefined;
...
let warnedVertexAlphaFlag = false;

// Perspective far/near cap: 24-bit depth then resolves ~1 mm at 15 m in a 20 m scene.
const NEAR_FAR_MAX_RATIO = 1000;

/**
 * Depth fields of a material payload, shared by `_createMaterial` and
 * `add_mesh_binary` so both honour `depthWrite` and `polygonOffset*` alike.
 * @param {any} params
 * @param {boolean} defaultDepthWrite
 * @returns {{fields: any, depthWriteExplicit: boolean}}
 */
function depthMaterialFields(params, defaultDepthWrite) {
    const depthWriteExplicit = params.depthWrite != null;
    /** @type {any} */
    const fields = {
        depthWrite: depthWriteExplicit ? params.depthWrite !== false : defaultDepthWrite,
    };
    if (params.polygonOffset) {
        fields.polygonOffset = true;
        fields.polygonOffsetFactor = params.polygonOffsetFactor != null ? params.polygonOffsetFactor : 0;
        fields.polygonOffsetUnits = params.polygonOffsetUnits != null ? params.polygonOffsetUnits : 0;
    }
    return { fields, depthWriteExplicit };
}
...
function applyOpacity(obj, opacity) {
    ...
    obj.traverse(/** @param {any} child */ child => {
        ...
        for (const mat of mats) {
            ...
            mat.opacity = opacity;
            // An explicit params.depthWrite outranks the opacity rule (#207).
 ...
 *    is exactly this case when the caller passed an explicit
...
void main() {
    ...
    // Axis lines through the local origin: a distinct colour at the same width,
    // and only when a centre colour was asked for (uCenterLines).
    vec2 axisDist = abs(coord) / fw;
    float centerA = uCenterLines
        * (1.0 - smoothstep(halfW - 0.5, halfW + 0.5, min(axisDist.x, axisDist.y)));
...
function buildFloorGridMesh(data) {
    ...
    const material = new THREE.ShaderMaterial({
        ...
        uniforms: {
            ...
            uCenterLines: { value: data.centerColor != null ? 1.0 : 0.0 },
...
// ========== Billboards ==========
//
// "Orient to viewer" is a per-object property, not a kind of object: any
// tracked object can carry it (`set_billboard`), and `add_billboard` is just a
// plane that comes with it switched on.
//
// Every mode is one composition, applied per rendered frame:
//
//     R_final = R_aim · R_own
//
// R_own is the object's own rotation (authored, or driven by an animation
// channel). It acts FIRST, in the object's local frame, which is what lets a
// wheel keep spinning about its own axle while the axle is aimed at the
// camera: a spin about an axis leaves that axis fixed, so the spin never
// fights the aim. The modes differ only in how many degrees of freedom the
// viewer claims — see BILLBOARD_MODES.

/**
 * `camera`   3 DOF, own rotation overridden — copies the camera's orientation,
 *            so the object stays parallel to the view plane and does not swing
 *            as it moves off-axis. Screen-aligned labels.
 * `world_up` 3 DOF, own rotation overridden — aims `face` at the camera's
 *            actual position, roll taken from `worldUp`. Stays upright.
 * `aim`      2 DOF, own rotation kept as roll — minimal rotation putting
 *            `face` on the camera. The wheel.
 * `hinge`    1 DOF, own rotation kept — spins about `hinge` only (held along
 *            `hingeWorld` when given), turning to aim `face` as best it can.
 */
const BILLBOARD_MODES = ['camera', 'world_up', 'aim', 'hinge'];

/** @type {Record<string, [number, number, number]>} */
const BILLBOARD_AXIS_NAMES = {
    '+x': [1, 0, 0], '-x': [-1, 0, 0],
    '+y': [0, 1, 0], '-y': [0, -1, 0],
    '+z': [0, 0, 1], '-z': [0, 0, -1],
};
...
const _bbParentQuatInv = new THREE.Quaternion();
const _bbAnchor = new THREE.Vector3();
const _bbPivotScaled = new THREE.Vector3();
const _bbDir = new THREE.Vector3();
const _bbTargetFace = new THREE.Vector3();
const _bbTargetUp = new THREE.Vector3();
const _bbU = new THREE.Vector3();
const _bbTmp = new THREE.Vector3();
const _bbPos = new THREE.Vector3();
const _bbHinge = new THREE.Vector3();
const _bbAxis = new THREE.Vector3();
const _bbV0 = new THREE.Vector3();
const _bbV1 = new THREE.Vector3();
const _bbCross = new THREE.Vector3();
const _bbAlign = new THREE.Quaternion();
const _bbAim = new THREE.Quaternion();
const _bbFinal = new THREE.Quaternion();
const _bbWorldQuat = new THREE.Quaternion();
const _bbSrcBasis = new THREE.Matrix4();
const _bbDstBasis = new THREE.Matrix4();
const _bbRotMat = new THREE.Matrix4();
const _bbBasisX = new THREE.Vector3();
const _bbBasisY = new THREE.Vector3();
const _bbBasisZ = new THREE.Vector3();

/**
 * Resolve an axis given as an axis name (`"+z"`, `"-x"`) or a 3-vector.
 *
 * Python normalizes these before sending, but `handleMessage` is a public
 * no-WS embedder surface: a zero or non-finite axis survives normalize() as a
 * zero/NaN vector and would feed a degenerate basis into the aim math every
 * frame. Fall back to the documented default instead, loudly. Options are
 * resolved once per set_billboard/add_billboard message, so a bad value warns
 * once per message — not once per object, and not once per frame.
 *
 * @param {unknown} value
 ...
 */
function parseBillboardAxis(value, fallback, what, id) {
    if (value == null) return new THREE.Vector3().fromArray(fallback);
    if (typeof value === 'string') {
        const key = value.trim().toLowerCase();
        const named = BILLBOARD_AXIS_NAMES[key] || BILLBOARD_AXIS_NAMES['+' + key];
        if (named) return new THREE.Vector3().fromArray(named);
    } else if (Array.isArray(value) && value.length === 3 && value.every(Number.isFinite)
        && (value[0] !== 0 || value[1] !== 0 || value[2] !== 0)) {
        return new THREE.Vector3().fromArray(value).normalize();
    }
    console.warn(
        `billboard '${id}': ${what} must be an axis name or a finite non-zero 3-vector ` +
        `(got ${JSON.stringify(value)}); using ${JSON.stringify(fallback)}.`
    );
    return new THREE.Vector3().fromArray(fallback);
}

/**
 * The pivot is a *point*, so zero is legal and it must not be normalized.
 * @param {unknown} value
 * @param {string} id
 * @returns {THREE.Vector3}
 */
function parseBillboardPivot(value, id) {
    if (value == null) return new THREE.Vector3();
    if (Array.isArray(value) && value.length === 3 && value.every(Number.isFinite)) {
        return new THREE.Vector3().fromArray(value);
    }
    console.warn(
        `billboard '${id}': pivot must be a finite 3-vector ` +
        `(got ${JSON.stringify(value)}); using [0, 0, 0].`
    );
    return new THREE.Vector3();
}

/**
 * Normalize an `add_billboard` / `set_billboard` payload into resolved options.
 * @param {any} data
 * @param {string} id
 */
function resolveBillboardOptions(data, id) {
    let mode = typeof data.mode === 'string' ? data.mode : 'camera';
    if (!BILLBOARD_MODES.includes(mode)) {
        console.warn(
            `billboard '${id}': unknown mode ${JSON.stringify(data.mode)} ` +
            `(expected one of ${BILLBOARD_MODES.join(', ')}); using 'camera'.`
        );
        mode = 'camera';
    }
    return {
        mode,
        face: parseBillboardAxis(data.face, [0, 0, 1], 'face', id),
        up: parseBillboardAxis(data.up, [0, 1, 0], 'up', id),
        worldUp: parseBillboardAxis(data.world_up, [0, 0, 1], 'world_up', id),
        hinge: parseBillboardAxis(data.hinge, [0, 1, 0], 'hinge', id),
        hingeWorld: data.hinge_world == null
            ? null
            : parseBillboardAxis(data.hinge_world, [0, 0, 1], 'hinge_world', id),
        pivot: parseBillboardPivot(data.pivot, id),
    };
}

/**
 * Orthonormal basis whose columns are (a, b orthogonalized against a, a×b), so
 * the matrix maps +X onto `a` and +Y onto the in-plane part of `b`.
 * @param {THREE.Vector3} a
 * @param {THREE.Vector3} b
 * @param {THREE.Matrix4} out
 * @returns {boolean} false when the pair is degenerate (parallel or zero)
 */
function billboardBasis(a, b, out) {
    _bbBasisX.copy(a);
    if (_bbBasisX.lengthSq() < 1e-20) return false;
    _bbBasisX.normalize();
    _bbBasisY.copy(b).addScaledVector(_bbBasisX, -b.dot(_bbBasisX));
    if (_bbBasisY.lengthSq() < 1e-12) return false;
    _bbBasisY.normalize();
    _bbBasisZ.crossVectors(_bbBasisX, _bbBasisY);
    out.makeBasis(_bbBasisX, _bbBasisY, _bbBasisZ);
    return true;
}

/**
 * 1-DOF hinge aim: the rotation about the hinge axis that best points `u`
 * (the object's current facing, in parent space) at `dir`.
 *
 * With `hingeWorld` the hinge is first aligned to that world direction and the
 * spin happens about it; without, the hinge stays wherever the object's own
 * rotation puts it.
 *
 * @param {{hinge: THREE.Vector3, hingeWorld: THREE.Vector3 | null}} opts
 * @param {{quaternion: THREE.Quaternion}} base
 * @param {THREE.Quaternion} parentQuatInv
 * @param {THREE.Vector3} dir facing target, parent space
 * @param {THREE.Vector3} u current facing, parent space
 * @param {THREE.Quaternion} out
 * @returns {boolean} false when the spin cannot aim at all (face ∥ hinge)
 */
function computeHingeAim(opts, base, parentQuatInv, dir, u, out) {
    _bbHinge.copy(opts.hinge).applyQuaternion(base.quaternion).normalize();
    if (opts.hingeWorld) {
        _bbAxis.copy(opts.hingeWorld).applyQuaternion(parentQuatInv).normalize();
        _bbAlign.setFromUnitVectors(_bbHinge, _bbAxis);
    } else {
        _bbAxis.copy(_bbHinge);
        _bbAlign.identity();
    }
    _bbV0.copy(u).applyQuaternion(_bbAlign).projectOnPlane(_bbAxis);
    _bbV1.copy(dir).projectOnPlane(_bbAxis);
    // face ∥ hinge (no spin can aim it) or camera on the hinge axis.
    if (_bbV0.lengthSq() < 1e-12 || _bbV1.lengthSq() < 1e-12) return false;
    _bbV0.normalize();
    _bbV1.normalize();
    const angle = Math.atan2(
        _bbCross.crossVectors(_bbV0, _bbV1).dot(_bbAxis),
        _bbV0.dot(_bbV1)
    );
    out.setFromAxisAngle(_bbAxis, angle).multiply(_bbAlign);
    return true;
}

/**
 * Record the pose a producer just wrote, as the base the billboard composes on.
 *
 * Called from every transform writer. The composed result must NEVER be read
 * back as R_own: `_updateBillboards` runs every frame but `_applyFrame` only
 * runs while an animation is *playing*, so a paused-then-orbited animation
 * would integrate a fresh R_aim per frame into unbounded runaway.
 *
 * No-op for objects that are not billboards, so the writers pay one property
 * read.
 *
 * @param {any} obj
 */
function captureBillboardBase(obj) {
    if (!obj || obj.userData.__isBillboard !== true) return;
    let base = obj.userData.__billboardBase;
    if (!base) {
        base = obj.userData.__billboardBase = {
    ...
    if (obj.matrixAutoUpdate) {
        base.position.copy(obj.position);
        base.quaternion.copy(obj.quaternion);
        base.scale.copy(obj.scale);
    } else {
        // The transforms channel writes obj.matrix and never touches the
        // components, so obj.scale is stale here — decompose instead.
        obj.matrix.decompose(base.position, base.quaternion, base.scale);
    }
}

/**
 * Turn the property off and restore the pose the producer last set.
 * @param {any} obj
 */
function disableBillboard(obj) {
    const base = obj.userData.__billboardBase;
    if (base) {
        if (obj.matrixAutoUpdate) {
            ...
            obj.matrix.compose(base.position, base.quaternion, base.scale);
            obj.matrixWorldNeedsUpdate = true;
        }
    }
    delete obj.userData.__isBillboard;
    delete obj.userData.__billboardOpts;
    delete obj.userData.__billboardBase;
    delete obj.userData.__billboardHold;
    delete obj.userData.__billboardDepth;
}

/**
 * Re-orient one billboard toward the camera. Called every frame from
 * `_updateBillboards`, parents before children.
 * @param {any} obj
 * @param {THREE.Camera} camera
 */
function applyBillboard(obj, camera) {
    const opts = obj.userData.__billboardOpts;
    const base = obj.userData.__billboardBase;
    if (!opts || !base) return;

    if (obj.parent) obj.parent.getWorldQuaternion(_bbParentQuat);
    else _bbParentQuat.identity();
    _bbParentQuatInv.copy(_bbParentQuat).invert();

    // Anchor = the pivot point under the BASE pose, in world space. Deriving it
    // from the base rather than the live transform keeps the aim a pure
    // function of producer state + camera, so it cannot drift into its own
    // output even with a non-zero pivot moving the object.
    _bbPivotScaled.copy(opts.pivot).multiply(base.scale);
    _bbAnchor.copy(_bbPivotScaled).applyQuaternion(base.quaternion).add(base.position);
    if (obj.parent) _bbAnchor.applyMatrix4(obj.parent.matrixWorld);

    camera.getWorldPosition(_bbCamPos);
    let ok = true;

    if (opts.mode === 'camera' || opts.mode === 'world_up') {
        // 3 DOF: the pose is fully determined and the object's own rotation is
        // overridden.
        if (opts.mode === 'camera') {
            camera.getWorldQuaternion(_bbCamQuat);
            _bbTargetFace.set(0, 0, 1).applyQuaternion(_bbCamQuat);
            _bbTargetUp.set(0, 1, 0).applyQuaternion(_bbCamQuat);
        } else {
            _bbTargetFace.subVectors(_bbCamPos, _bbAnchor);
            ok = _bbTargetFace.lengthSq() > 1e-20;
            if (ok) {
                _bbTargetFace.normalize();
                _bbTargetUp.copy(opts.worldUp);
            }
        }
        if (ok) {
            ok = billboardBasis(opts.face, opts.up, _bbSrcBasis)
                && billboardBasis(_bbTargetFace, _bbTargetUp, _bbDstBasis);
        }
        if (ok) {
            // R_world = T · Sᵀ maps face -> targetFace and up -> targetUp.
            _bbRotMat.multiplyMatrices(_bbDstBasis, _bbSrcBasis.transpose());
            _bbWorldQuat.setFromRotationMatrix(_bbRotMat);
            _bbFinal.copy(_bbParentQuatInv).multiply(_bbWorldQuat);
        }
    } else {
        // R_final = R_aim · R_own — the own rotation acts first, in local space.
        _bbDir.subVectors(_bbCamPos, _bbAnchor);
        ok = _bbDir.lengthSq() > 1e-20;
        if (ok) {
            _bbDir.normalize().applyQuaternion(_bbParentQuatInv);
            _bbU.copy(opts.face).applyQuaternion(base.quaternion);
            if (opts.mode === 'aim') {
                _bbAim.setFromUnitVectors(_bbU, _bbDir);
            } else {
                ok = computeHingeAim(opts, base, _bbParentQuatInv, _bbDir, _bbU, _bbAim);
            }
            if (ok) _bbFinal.copy(_bbAim).multiply(base.quaternion);
        }
    }

    if (ok) {
        let hold = obj.userData.__billboardHold;
        if (!hold) hold = obj.userData.__billboardHold = new THREE.Quaternion();
        hold.copy(_bbFinal);
    } else {
        // Degenerate this frame (camera on the hinge axis, face ∥ hinge).
        // Re-apply the last good pose rather than skipping the write: an
        // animated object's matrix was just overwritten with the *un-aimed*
        // pose, so skipping would flash the base orientation.
        const hold = obj.userData.__billboardHold;
        if (!hold) return;
        _bbFinal.copy(hold);
    }

    // Pivot: keep the anchor point exactly where the base pose put it.
    //   P_final = P_own + R_own·(S∘pivot) − R_final·(S∘pivot)
    _bbPos.copy(base.position);
    if (opts.pivot.lengthSq() > 0) {
        _bbTmp.copy(_bbPivotScaled).applyQuaternion(base.quaternion);
        _bbPos.add(_bbTmp);
        _bbTmp.copy(_bbPivotScaled).applyQuaternion(_bbFinal);
        _bbPos.sub(_bbTmp);
    }

    if (obj.matrixAutoUpdate) {
        obj.quaternion.copy(_bbFinal);
        obj.position.copy(_bbPos);
    } else {
        // Animation / follow-path own obj.matrix here: a component write would
        // be ignored, because updateMatrix() is never called. (That is the bug
        // that made an animated billboard silently not billboard.)
        obj.matrix.compose(_bbPos, _bbFinal, base.scale);
        obj.matrixWorldNeedsUpdate = true;
    }
}
...
class CameraController {
    ...
    updateNearFar() {
        ...
        // precision back).
        const fitFar = dist + radius * 1.5;
        const nextFar = Math.max(fitFar, 100);
        // Inside the sphere dist - radius is <= 0; a near tied to the fitted far keeps
        // depth precision (issue #227), capped so the orbit target never clips.
        const targetDist = v._perspCamera.position.distanceTo(v._controls.target);
        const nextNear = Math.max(
            0.001,
            (dist - radius) * 0.5,
            Math.min(fitFar / NEAR_FAR_MAX_RATIO, targetDist * 0.5),
        );
        if (
...
//   functions; refresh() re-evaluates them. A `shortcut` ("G", "Shift+G")
//   is shown as a key chip AND bound (user-defined keyboard shortcuts) unless
//   `bindKey: false`; keys the viewer itself uses are refused with a warning. `eye` items own every tracked
...
// Animated eye/eye-off glyph: one eye-path masked by a polyline that sweeps
// a diagonal cut across it, plus a separate slash line drawn on top — both
// are SMIL <animate>s with begin="indefinite", fired via beginElement() by
// playEyeTransition() so toggling *animates* between states instead of
// swapping a static icon. The un-animated markup already rests in the
// "open" (visible) pose, so an eye that starts visible needs no JS before
// its first paint; setEyeRestState() jumps a closed eye there without
// playing the animation. Mask/element ids are suffixed per instance
// (`_eyeUidSeq`) because `url(#id)` resolves document-wide, and two eye
// items on screen at once would otherwise fight over one mask.
let _eyeUidSeq = 0;
function eyeSvgMarkup() {
    const mask = `tjsv-eyeCutMask-${++_eyeUidSeq}`;
    return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
<mask id="${mask}" maskUnits="userSpaceOnUse" x="0" y="0" width="24" height="24">
<rect width="24" height="24" fill="#fff"/>
<polyline class="tjsv-eye-cut" points="12,12 12,12" fill="none" stroke="#000" stroke-width="6" stroke-linecap="round">
<animate class="off" attributeName="points" begin="indefinite" dur="0.4s" fill="freeze" calcMode="spline" keyTimes="0;1" keySplines=".4 0 .2 1" values="12,12 12,12;3,20 21,4"/>
<animate class="on" attributeName="points" begin="indefinite" dur="0.4s" fill="freeze" calcMode="spline" keyTimes="0;1" keySplines=".4 0 .2 1" values="3,20 21,4;12,12 12,12"/>
</polyline>
</mask>
<path mask="url(#${mask})" d="M2 12C4.5 7 8 5 12 5C16 5 19.5 7 22 12C19.5 17 16 19 12 19C8 19 4.5 17 2 12Z"/>
<polyline class="tjsv-eye-slash" points="12,12 12,12" fill="none" stroke-width="6">
<animate class="off" attributeName="points" begin="indefinite" dur="0.4s" fill="freeze" calcMode="spline" keyTimes="0;1" keySplines=".4 0 .2 1" values="12,12 12,12;3,20 21,4"/>
<animate class="off" attributeName="stroke-width" begin="indefinite" dur="0.4s" fill="freeze" calcMode="spline" keyTimes="0;1" keySplines=".4 0 .2 1" values="6;2"/>
<animate class="on" attributeName="points" begin="indefinite" dur="0.4s" fill="freeze" calcMode="spline" keyTimes="0;1" keySplines=".4 0 .2 1" values="3,20 21,4;12,12 12,12"/>
<animate class="on" attributeName="stroke-width" begin="indefinite" dur="0.4s" fill="freeze" calcMode="spline" keyTimes="0;1" keySplines=".4 0 .2 1" values="2;6"/>
</polyline>
</svg>`;
}

/**
 * Jump an eye glyph straight to its resting pose with no animation — used
 * on the first paint, where playEyeTransition() would instead animate from
 * the markup's baked-in "open" default. @param {Element} mark @param {boolean} on
 */
function setEyeRestState(mark, on) {
    if (on) return; // markup already rests open
    const cut = mark.querySelector('.tjsv-eye-cut');
    const slash = mark.querySelector('.tjsv-eye-slash');
    if (cut) cut.setAttribute('points', '3,20 21,4');
    if (slash) { slash.setAttribute('points', '3,20 21,4'); slash.setAttribute('stroke-width', '2'); }
}

/**
 * Play the open<->closed transition baked into the glyph's SMIL animations.
 * @param {Element} mark @param {boolean} on
 */
function playEyeTransition(mark, on) {
    const dir = on ? 'on' : 'off';
    for (const anim of /** @type {NodeListOf<SVGAnimationElement>} */ (mark.querySelectorAll(`animate.${dir}`))) {
        anim.beginElement();
    }
}
...
class MenuController {
    ...
    _makeHandle(m) {
        ...
        return {
            ...
            applyEyes: () => self.applyEyes(m),
            /** @param {string} itemId */
            applyEye: (itemId) => {
                const rec = m.items.get(itemId);
                if (rec && (rec.spec.type || '') === 'eye') self._applyEye(rec.spec, !!self._valueOf(m, rec.spec));
            },
        };
    }
    ...
    _buildItem(m, it) {
        ...
        const rec = { spec: it, el: /** @type {HTMLElement|null} */ (null), input: /** @type {any} */ (null), shortcut: null, eyeState: /** @type {boolean|undefined} */ (undefined) };
        let el;
        if (type === 'divider') {
        ...
        rec.el = el;
        // A client item's shortcut is bound unless it opts out; the built-in
        // menu's keys are handled by the viewer's own keydown switch.
        const bind = it.shortcut && (it.bindKey !== undefined ? !!it.bindKey : !m.spec.builtin);
        if (bind) {
    ...
    _paint(m, rec) {
        ...
        if (type === 'button') {
            ...
            if (type === 'eye') {
                ...
                if (mark) {
                    if (rec.eyeState === undefined) setEyeRestState(mark, on);
                    else if (rec.eyeState !== on) playEyeTransition(mark, on);
                    rec.eyeState = on;
                }
            }
            const st = el.querySelector('.tjsv-menu-state');
    ...
    _eyeOwns(it, objId) {
        if (typeof it.match === 'function') return !!it.match(objId);
...
// Same base/hover colour pairs as examples/36_axis_control.py's RED/GREEN/BLUE,
...
const GIZMO_PLANE_PICK_THICKNESS = 0.02;   // small box depth along the plane's normal, for a forgiving hitbox
...
/** TransformControls handle name → default hover-tint hex, or null. */
function gizmoAxisHoverColor(name) {
    if (name === 'X' || name === 'YZ') return GIZMO_HOVER_PALETTE.x;
    if (name === 'Y' || name === 'XZ') return GIZMO_HOVER_PALETTE.y;
    if (name === 'Z' || name === 'XY') return GIZMO_HOVER_PALETTE.z;
    return null;
}

// Which handle names light up together when `name` is hovered/dragging — the
// centre (free move) highlights everything, a plane highlights itself plus
// the two axes that span it, a single axis highlights only itself.
const GIZMO_HIGHLIGHT_ALL = new Set(['X', 'Y', 'Z', 'XY', 'YZ', 'XZ']);
const GIZMO_HIGHLIGHT_BY_NAME = {
    XYZ: GIZMO_HIGHLIGHT_ALL,
    XY: new Set(['XY', 'X', 'Y']),
    YZ: new Set(['YZ', 'Y', 'Z']),
    XZ: new Set(['XZ', 'X', 'Z']),
    X: new Set(['X']), Y: new Set(['Y']), Z: new Set(['Z']),
};
/** @param {string|null} name  the live `TransformControls.axis` @returns {Set<string>|null} */
function gizmoHighlightSet(name) {
    return name ? (GIZMO_HIGHLIGHT_BY_NAME[name] || null) : null;
}
...
/** @param {number|string} hex @returns {THREE.Color} */
...
// Lazily-derived, cached "hovered" tint (lighten, same recipe as the axis
// control widget's default hover colour) for a base hex with no explicit
// hoverColor override.
const _gizmoHoverColorCache = new Map();
/** @param {number|string} hex @returns {THREE.Color} */
function gizmoHoverColor(hex) {
    let c = _gizmoHoverColorCache.get(hex);
    if (!c) { c = gizmoColor(hex).clone().offsetHSL(0, 0, AXIS_CONTROL_HOVER_LIGHTEN); _gizmoHoverColorCache.set(hex, c); }
    return c;
}
...
//   3. For the move gizmo only (`opts.customTranslateHandles`, the clip
//      gizmos don't pass it): replace the stock X/Y/Z arrows with a simple
//      fat line each (axis-control-widget styling) with a pill-shaped
//      (capsule) picker around it, replace the centre free-move handle's
//      picker with a sphere and hide its visible mesh entirely (it renders
//      nothing — only the plane chips and axis lines stay visible), and
//      leave the plane chips' own geometry untouched (`restyleGizmoHelper`
//      already resizes/margins them into a clean box shape). The rotate
...
// Legacy stock-arrow picker tuning, kept only for the clip gizmos (which
// don't opt into `customTranslateHandles`).
...
/** @param {string} name  'X'|'Y'|'Z' @returns {[number, number, number]} */
function gizmoAxisUnitVector(name) {
    return name === 'X' ? [1, 0, 0] : name === 'Y' ? [0, 1, 0] : [0, 0, 1];
}

/** The two axes spanning a plane handle, in a fixed (u, v) order.
 * @param {string} name  'XY'|'YZ'|'XZ' @returns {{u:[number,number,number], v:[number,number,number]}} */
function gizmoPlaneAxes(name) {
    if (name === 'XY') return { u: [1, 0, 0], v: [0, 1, 0] };
    if (name === 'YZ') return { u: [0, 1, 0], v: [0, 0, 1] };
    return { u: [1, 0, 0], v: [0, 0, 1] };   // 'XZ'
}

/** Simple fat line from the gizmo centre out to `GIZMO_AXIS_LENGTH * scale`
 * along `name` (the three axes meet at the centre) — same linewidth
 * convention as the axis-control widget's guide line.
 * @param {string} name  'X'|'Y'|'Z' @param {number} scale @returns {any} a Line2 */
function buildGizmoAxisLine(name, scale) {
    const [ux, uy, uz] = gizmoAxisUnitVector(name);
    const len = GIZMO_AXIS_LENGTH * scale;
    const geo = new LineGeometry();
    geo.setPositions([0, 0, 0, len * ux, len * uy, len * uz]);
    const mat = new LineMaterial({
        linewidth: AXIS_CONTROL_LINE_WIDTH_PX, toneMapped: false,   // depth-tested within the gizmo overlay pass
    });
...
/** Pill-shaped (capsule) hitbox around one axis line, centre to tip.
 * @param {string} name  'X'|'Y'|'Z' @param {number} scale @returns {any} a Mesh */
function buildGizmoAxisPicker(name, scale) {
    const len = GIZMO_AXIS_LENGTH * scale;
    const r = GIZMO_AXIS_PICK_RADIUS * scale;
    const cylLen = Math.max(len - 2 * r, 0.01);
    const geo = new THREE.CapsuleGeometry(r, cylLen, 4, 8);   // default long axis is local Y
    geo.translate(0, len / 2, 0);
    if (name === 'X') geo.rotateZ(-Math.PI / 2);
    else if (name === 'Z') geo.rotateX(Math.PI / 2);
    const mesh = new THREE.Mesh(geo, new THREE.MeshBasicMaterial());
    mesh.name = name;
    // Raycast-only hitbox — three's raycaster ignores `.visible` (same trick
    // as the axis-control widget's own invisible pick sphere).
    mesh.visible = false;
    return mesh;
}

/** A flat (or, for a picker, thin-boxed) square plane chip, offset
 * `GIZMO_PLANE_GAP_FACTOR * axis length` from the gizmo centre along +u/+v
 * (a gap from the axis lines) and `GIZMO_PLANE_SIZE_FACTOR * axis length`
 * on a side.
 * @param {string} name  'XY'|'YZ'|'XZ' @param {number} scale @param {number} thickness  0 for a flat quad
 * @returns {any} a BufferGeometry */
function buildGizmoPlaneGeometry(name, scale, thickness) {
    const { u, v } = gizmoPlaneAxes(name);
    const uVec = new THREE.Vector3(...u);
    const vVec = new THREE.Vector3(...v);
    const nVec = new THREE.Vector3().crossVectors(uVec, vVec).normalize();
    const basis = new THREE.Matrix4().makeBasis(uVec, vVec, nVec);
    const axisLen = GIZMO_AXIS_LENGTH * scale;
    const gap = axisLen * GIZMO_PLANE_GAP_FACTOR;
    const side = axisLen * GIZMO_PLANE_SIZE_FACTOR;
    const geo = thickness > 0
        ? new THREE.BoxGeometry(side, side, thickness * scale)
        : new THREE.PlaneGeometry(side, side);
    geo.translate(gap + side / 2, gap + side / 2, 0);   // near corner `gap` out from centre, extends toward +u, +v
    geo.applyMatrix4(basis);
    return geo;
}

/** @param {string} name  'XY'|'YZ'|'XZ' @param {number} scale @returns {any} a Mesh */
function buildGizmoPlaneMesh(name, scale) {
    const mesh = new THREE.Mesh(buildGizmoPlaneGeometry(name, scale, 0), new THREE.MeshBasicMaterial({
        toneMapped: false, side: THREE.DoubleSide,   // opaque, depth-tested within the gizmo overlay pass
    }));
    mesh.name = name;
    return mesh;
}
...
/** @param {string} name  'XY'|'YZ'|'XZ' @param {number} scale @returns {any} a Mesh */
function buildGizmoPlanePicker(name, scale) {
    const mesh = new THREE.Mesh(buildGizmoPlaneGeometry(name, scale, GIZMO_PLANE_PICK_THICKNESS), new THREE.MeshBasicMaterial());
    mesh.name = name;
    mesh.visible = false;   // raycast-only, see buildGizmoAxisPicker
    return mesh;
}

/** Replace the stock X/Y/Z arrows, plane chips, and centre handle with
 * axis-control-style lines/planes/pickers (issue: "look like the axis
 * control widget"). @param {any} gm  control._gizmo @param {number} scale */
function buildGizmoTranslateHandles(gm, scale) {
    const visible = gm.gizmo && gm.gizmo.translate;
    const picker = gm.picker && gm.picker.translate;
    const AXES = ['X', 'Y', 'Z'];
    const PLANES = ['XY', 'YZ', 'XZ'];
    if (visible) {
        for (const child of visible.children.slice()) {
            if (AXES.includes(child.name) || PLANES.includes(child.name)) visible.remove(child);
            else if (child.name === 'XYZ') child.visible = false;   // centre renders nothing
        }
        for (const name of AXES) visible.add(buildGizmoAxisLine(name, scale));
        for (const name of PLANES) visible.add(buildGizmoPlaneMesh(name, scale));
    }
    if (picker) {
        for (const child of picker.children.slice()) {
            if (AXES.includes(child.name) || PLANES.includes(child.name)) picker.remove(child);
            else if (child.name === 'XYZ' && child.geometry) {
                child.geometry.dispose();
                child.geometry = new THREE.SphereGeometry(GIZMO_CENTER_PICK_RADIUS * scale, 12, 8);
            }
        }
        for (const name of AXES) picker.add(buildGizmoAxisPicker(name, scale));
        for (const name of PLANES) picker.add(buildGizmoPlanePicker(name, scale));
    }
    // Stock TransformControls extends a long dashed "AXIS" guide line through
    // whichever handle is being dragged — with the whole handle now just a
    // simple line, that read as an infinite line snapping into view on
    // selection. Only three axis lines + three plane chips should ever render.
    const helperT = gm.helper && gm.helper.translate;
    if (helperT) {
        for (const child of helperT.children.slice()) helperT.remove(child);
    }
}

// Stock rotate rings are half-circle tori (radius 0.5) whose orientation is
// baked into the geometry by `setupGizmo` — X: none, Y: Euler(0, 0, -π/2),
// Z: Euler(0, π/2, 0), on top of the lib's CircleGeometry rotateY(π/2) +
// rotateX(π/2). TransformControls then spins each handle about its own axis
// every frame so the half-ring faces the camera. We bake the same transforms
// into our own arc so that per-frame spin still lines up.
const GIZMO_ROTATE_BAKED_EULER = { X: [0, 0, 0], Y: [0, 0, -Math.PI / 2], Z: [0, Math.PI / 2, 0] };
const GIZMO_ROTATE_ARC_SEGMENTS = 48;

/** The stock CircleGeometry + setupGizmo orientation for rotate handle `name`.
 * @param {string} name  'X'|'Y'|'Z' @returns {THREE.Matrix4} */
function gizmoRotateBakeMatrix(name) {
    const [ex, ey, ez] = GIZMO_ROTATE_BAKED_EULER[/** @type {'X'|'Y'|'Z'} */ (name)];
    const baked = new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(ex, ey, ez));
    const circle = new THREE.Matrix4().makeRotationX(Math.PI / 2)
        .multiply(new THREE.Matrix4().makeRotationY(Math.PI / 2));
    return baked.multiply(circle);
}

/** Half-circle fat-line arc, radius = axis length, same linewidth as the
 * axis lines. @param {string} name  'X'|'Y'|'Z' @param {number} scale @returns {any} a Line2 */
function buildGizmoRotateArc(name, scale) {
    const r = GIZMO_AXIS_LENGTH * scale;
    const m = gizmoRotateBakeMatrix(name);
    const p = new THREE.Vector3();
    const pos = [];
    for (let i = 0; i <= GIZMO_ROTATE_ARC_SEGMENTS; i++) {
        const t = (i / GIZMO_ROTATE_ARC_SEGMENTS) * Math.PI;
        p.set(r * Math.cos(t), r * Math.sin(t), 0).applyMatrix4(m);
...
/** Half-torus hitbox hugging the visible arc (same bake, so the lib's
 * per-frame spin keeps the two aligned). @param {string} name @param {number} scale @returns {any} a Mesh */
function buildGizmoRotatePicker(name, scale) {
    const geo = new THREE.TorusGeometry(GIZMO_AXIS_LENGTH * scale, GIZMO_AXIS_PICK_RADIUS * scale, 4, 24, Math.PI);
    geo.applyMatrix4(gizmoRotateBakeMatrix(name));
    const mesh = new THREE.Mesh(geo, new THREE.MeshBasicMaterial());
    mesh.name = name;
    mesh.visible = false;   // raycast-only, see buildGizmoAxisPicker
    return mesh;
...
/** Replace the stock X/Y/Z rotate rings (and their pickers) with arcs styled
 * like the translate axis lines. @param {any} gm  control._gizmo @param {number} scale */
function buildGizmoRotateHandles(gm, scale) {
    const AXES = ['X', 'Y', 'Z'];
    const visible = gm.gizmo && gm.gizmo.rotate;
    if (visible) {
        for (const child of visible.children.slice()) if (AXES.includes(child.name)) visible.remove(child);
        for (const name of AXES) visible.add(buildGizmoRotateArc(name, scale));
    }
    const picker = gm.picker && gm.picker.rotate;
    if (picker) {
        for (const child of picker.children.slice()) if (AXES.includes(child.name)) picker.remove(child);
        for (const name of AXES) picker.add(buildGizmoRotatePicker(name, scale));
    }
}

// Refined look for one gizmo helper's handles. Runs every frame (cheap traverse)
// so our resting palette survives TransformControls' per-frame re-theme.
// Shared by the move gizmo and the clipping gizmos (which never have planar
// handles, so the planar-only branches below are inert for them).
/** @param {any} helper  a TransformControls helper Object3D
 *  @param {{control?: any, color?: number|string|null, hoverColor?: number|string|null, resW?: number, resH?: number}} [opts]
 *    `control`/`color`/`hoverColor` only ever passed by the move gizmo (never
 *    the clip gizmos): opts into live hover/drag highlighting, an optional
 *    colour override (falls back to the per-axis palette), fully-opaque axis
 *    lines (vs. the legacy 0.92 for stock arrows), and no plane outline. */
function restyleGizmoHelper(helper, opts = {}) {
    const highlightSet = opts.control ? gizmoHighlightSet(opts.control.axis) : null;
    ...
    helper.traverse((/** @type {any} */ o) => {
        ...
        if (moveGizmo && o.name === 'XYZ') {
            // The centre should render nothing (only its sphere picker is
            // real) — TransformControls re-asserts `.visible = true` on its
            // own handles every frame, so force this back off here too
            // rather than only once at construction.
            o.visible = false; m.opacity = 0; m.transparent = true;
            return;
        }
        const hex = gizmoAxisColor(o.name);
        if (hex == null) return;
        const hovered = !!(highlightSet && highlightSet.has(o.name));
        const baseHex = opts.color != null ? opts.color : hex;
        let col;
        if (!hovered) {
            col = gizmoColor(baseHex);             // cached; do not mutate
        } else if (opts.hoverColor != null) {
            col = gizmoColor(opts.hoverColor);
        } else if (opts.color != null) {
            col = gizmoHoverColor(baseHex);        // custom base colour: auto-lighten it
        } else {
            const hoverHex = gizmoAxisHoverColor(o.name);
            col = hoverHex != null ? gizmoColor(hoverHex) : gizmoHoverColor(baseHex);
        }
        m._color = (m._color || new THREE.Color()).copy(col);
        m.color.copy(col);
        if (planar) {
            // Slightly translucent flat chip, no outline.
            m._opacity = GIZMO_PLANE_OPACITY; m.opacity = GIZMO_PLANE_OPACITY; m.transparent = true;
        } else if (moveGizmo) {
            // Move-gizmo axis lines are fully opaque.
            m._opacity = 1.0; m.opacity = 1.0; m.transparent = false;
        } else {
...
/** @typedef {{x?:boolean|null, y?:boolean|null, z?:boolean|null}} GizmoAxisFlags */
/** @typedef {GizmoAxisFlags & {translate?:GizmoAxisFlags|null, rotate?:GizmoAxisFlags|null}} GizmoAxesMask */
...
class Gizmo {
    /** @param {TransformGizmoController} owner @param {{scale?: number}} [opts] */
    constructor(owner, opts = {}) {
        ...
        // Per-mode axis masks: showX/Y/Z is one flag set for both translate
        // arrows and rotate rings, so it is re-applied on every mode switch.
        this.axes = { translate: { x: true, y: true, z: true }, rotate: { x: true, y: true, z: true } };
        ...
        owner._refineHandles(this);  // strip the bulky rotate handles + build our own translate handles (one-time)
        owner._restyleGizmo(this);   // prime colour caches up front

        // TransformControlsGizmo re-themes its own handles (a hardcoded
        // yellow on whichever one matches `control.axis`) inside its own
        // `updateMatrixWorld`, which the renderer calls at the start of
        // every `render()` — strictly *after* our `_restyleGizmo` call in
        // `_animate`, so it would win the draw every time. Wrap it so our
        // colours (base + the plane/centre-aware hover highlight) are
        // re-applied immediately after the stock pass, same frame, before
        // anything gets drawn.
        const gizmoObj = /** @type {any} */ (ctrl._gizmo);
        if (gizmoObj && !gizmoObj.__restyleWrapped) {
            gizmoObj.__restyleWrapped = true;
    ...
    dispose() {
        ...
        v._gizmoScene.remove(this.helper);
    }
}
...
class TransformGizmoController {
    ...
    _onDragChange(g, dragging) {
        ...
        if (dragging) {
            ...
            if (g.object) {
                ...
                this._spawnGhost(g);
                // A billboard would re-aim the object every frame of the drag,
                // fighting the handle and reporting a camera-derived pose back
                // to the producer. The drag owns the transform until release.
                g.object.userData.__billboardDragging = true;
            }
        } else {
            if (g.object) {
                delete g.object.userData.__billboardDragging;
                // The drag IS the new producer pose: adopt it as the base, or
                // the billboard would snap back to the pre-drag transform.
                captureBillboardBase(g.object);
            }
            this._clearGhost(g);
            this._report(g, true);
        }
    ...
    setMode(mode) {
        ...
        this._setControlMode(this._primary, mode);
    ...
    _setControlMode(g, mode) {
        g.control.setMode(mode);
        this._syncAxes(g);
    }

    /** @param {Gizmo} g */
    _syncAxes(g) {
        const m = g.axes[g.control.getMode()] || g.axes.translate;
        g.control.showX = m.x;
        g.control.showY = m.y;
        g.control.showZ = m.z;
    }
     ...
     * Top-level `x`/`y`/`z` apply to both modes; a `translate` or `rotate`
     * sub-mask replaces them for that mode only (issue #220: keep every
     * translate arrow but hide one rotate ring). Additive — `_restyleGizmo()`
     * never touches `showX/Y/Z`.
     * @param {Gizmo} g @param {GizmoAxesMask|null} [mask]
     */
    _applyAxes(g, mask) {
        const m = mask || {};
        const resolve = (/** @type {{x?:boolean|null,y?:boolean|null,z?:boolean|null}} */ s) =>
            ({ x: s.x !== false, y: s.y !== false, z: s.z !== false });
        g.axes = { translate: resolve(m.translate || m), rotate: resolve(m.rotate || m) };
        this._syncAxes(g);
    }
    ...
    // highlighting) and the renderer size (fat-line resolution uniform).
    /** @param {Gizmo} g */
    _restyleGizmo(g) {
        const w = this.v.container.clientWidth, h = this.v.container.clientHeight;
        restyleGizmoHelper(g.helper, {
            control: g.control, color: g.color, hoverColor: g.hoverColor, resW: w, resH: h,
        });
    }
     ...
     * @param {{id?:string|null, mode?:string, axes?:GizmoAxesMask|null, space?:string, snapDefault?:boolean, color?:number|string|null, hoverColor?:number|string|null, scale?:number}} [opts]
     * @returns {Gizmo|null}
     */
    addGizmo(object, opts = {}) {
        if (!object) return null;
        this._activate();
        const g = new Gizmo(this, { scale: opts.scale });
        g.object = object;
        ...
        if (opts.hoverColor != null) g.hoverColor = opts.hoverColor;
        if (opts.space === 'local' || opts.space === 'world') g.control.setSpace(opts.space);
        g.control.attach(object);
        g.control.enabled = true;
        g.helper.visible = true;
        g.control.setMode(g.mode);
        this._applyAxes(g, opts.axes || null);  // also syncs showX/Y/Z to g.mode
        this._restyleGizmo(g);
        this._extra.push(g);
        return g;
    }
    ...
    enable(opts = {}) {
        ...
        if (opts.color != null) this._primary.color = opts.color;
        if (opts.hoverColor != null) this._primary.hoverColor = opts.hoverColor;
        this.clickSelect = (typeof opts.clickSelect === 'boolean') ? opts.clickSelect : true;
        this._activate();
        this._setControlMode(this._primary, this._primary.mode);
        if (opts.id != null) {
...
const AXIS_CONTROL_SPHERE_RADIUS = 0.1;    // invisible pick-only hitbox, not rendered
...
const AXIS_CONTROL_LINE_WIDTH_PX = 3;      // fat line (Line2/LineMaterial), same convention as the polyline path
const AXIS_CONTROL_HANDLE_WIDTH_PX = AXIS_CONTROL_LINE_WIDTH_PX * 4;  // handle segment: 4x the guide line
const AXIS_CONTROL_HANDLE_HALF_ANGLE = 0.05;   // radians either side of value, rotary handle arc
const AXIS_CONTROL_HANDLE_ARC_SEGMENTS = 12;
const AXIS_CONTROL_HANDLE_HALF_LENGTH = 0.05;  // world units either side of value, linear handle segment
...
class AxisControl {
    ...
    constructor(id, opts) {
        ...
        this._hoverColor = opts.hoverColor != null
        ...
        // GPUs regardless of `linewidth` (WebGL ignores it), too thin to read
        // as a real range indicator. `resolution` needs the render target's
        ...
        // The handle: a short, 3x-thick section of the same line, centred on
        // `value` — a small arc for rotary, a short segment for linear. This
        // is the visible "you are here" indicator; it shares its base/hover
        // colour with the long guide line (both update together in
        // `_setHovered`).
        ...
        this.handleLine.renderOrder = 901;
        this.handleLine.raycast = () => {};   // guide only — the sphere is the only pickable part

        // The sphere is kept as the pick target (three's raycaster ignores
        // `.visible`, so it still hit-tests) but is never rendered — the
        // handle line above is the visible indicator now.
        ...
        this.group.add(this.line, this.handleLine, this.sphere);
        this._rebuild();
    }

    /** Re-derive the line geometry (kind-dependent), the handle segment, and
     * the sphere's local position from the current value/min/max. Cheap
     * (<= ARC_SEGMENTS+1 verts) — called on every value/min/max change, not
     * per frame. */
    _rebuild() {
        ...
        if (isRotary) {
            const R = this.radius;
            // The guide never draws more than one turn: a wider range would
            // only overdraw itself. The value and its clamp keep the full range.
            const fullTurn = this.kind === 'rotary_unlimited' || this.max - this.min >= Math.PI * 2;
            const a0 = fullTurn ? 0 : this.min;
            const a1 = fullTurn ? Math.PI * 2 : this.max;
            for (let i = 0; i <= AXIS_CONTROL_ARC_SEGMENTS; i++) {
                const a = a0 + (a1 - a0) * (i / AXIS_CONTROL_ARC_SEGMENTS);
                flat.push(0, R * Math.cos(a), R * Math.sin(a));
            }
            this.sphere.position.set(0, R * Math.cos(this.value), R * Math.sin(this.value));

            let h0 = this.value - AXIS_CONTROL_HANDLE_HALF_ANGLE;
            let h1 = this.value + AXIS_CONTROL_HANDLE_HALF_ANGLE;
            if (this.kind === 'rotary') { h0 = Math.max(this.min, h0); h1 = Math.min(this.max, h1); }
            for (let i = 0; i <= AXIS_CONTROL_HANDLE_ARC_SEGMENTS; i++) {
                const a = h0 + (h1 - h0) * (i / AXIS_CONTROL_HANDLE_ARC_SEGMENTS);
                handleFlat.push(0, R * Math.cos(a), R * Math.sin(a));
            }
        } else {
            ...
            flat.push(lo, 0, 0, hi, 0, 0);
            this.sphere.position.set(this.value, 0, 0);

            let h0 = this.value - AXIS_CONTROL_HANDLE_HALF_LENGTH;
            let h1 = this.value + AXIS_CONTROL_HANDLE_HALF_LENGTH;
            if (this.kind === 'linear') { h0 = Math.max(this.min, h0); h1 = Math.min(this.max, h1); }
            handleFlat.push(h0, 0, 0, h1, 0, 0);
        }
        ...
        this.handleLine.geometry.dispose();
        this.handleLine.geometry = new LineGeometry();
        this.handleLine.geometry.setPositions(handleFlat);
        this.handleLine.computeLineDistances();
    }
    ...
    _setHovered(hovered) {
        ...
        const color = hovered ? this._hoverColor : this._baseColor;
        this.lineMaterial.color.copy(color);
        this.handleLineMaterial.color.copy(color);
    }

    /** Re-tint the guide line + handle. Omitting `hoverColor` re-derives it
     * from `color` (or the current base color) the same way the constructor
     * does — passing `color` alone still gives a sensible hover tint.
     * @param {number|string|null} [color] @param {number|string|null} [hoverColor] */
    setColor(color, hoverColor) {
        if (color != null) this._baseColor.set(color);
        this._hoverColor = hoverColor != null
            ? new THREE.Color(hoverColor)
            : this._baseColor.clone().offsetHSL(0, 0, AXIS_CONTROL_HOVER_LIGHTEN);
        const current = this._hovered ? this._hoverColor : this._baseColor;
        this.lineMaterial.color.copy(current);
        this.handleLineMaterial.color.copy(current);
    }
...
class AxisControlManager {
     ...
     * @param {string} id @param {{value?:number, min?:number, max?:number, color?:number|string|null, hoverColor?:number|string|null}} opts */
    setValue(id, opts) {
        const c = this.controls.get(id);
        if (!c) return;
        c.setValue(opts.value != null ? opts.value : c.value, opts);
        if (opts.color != null || opts.hoverColor != null) c.setColor(opts.color, opts.hoverColor);
    }
    ...
    update() {
        ...
        // Fat lines read their pixel width off this every frame; cheap (a
        // .set() on an existing Vector2, no realloc) so no resize hook needed.
        const w = this.v.container.clientWidth, h = this.v.container.clientHeight;
...
export class ThreeJSViewer {
    ...
    constructor(container, options = /** @type {ThreeJSViewerOptions} */ ({})) {
        ...
        // Keyed by object id, plus ANIMATION_FETCH_KEY (a Symbol) for the
        ...
        /** @type {any[] | null} Billboards ordered parents-first (see _billboardOrder). */
        this._billboardSorted = null;
        this._billboardOrderDirty = false;
        this._billboardOrderGen = -1;
        ...
        // fetched over HTTP as their messages arrive, so there is no
        // knowable up-front total. Instead the counters describe the current
        // *load batch*: a batch opens when _pendingFetches goes 0 -> 1 and
        // closes when it returns to 0. _loadBatchTotal is the number of
        // fetches started in that batch (a running count of what has been
        // seen, which may grow mid-batch — never a prediction), and
        // _loadBatchLoaded the number of them already finished. The
        // finished batch's counters are retained after the drain and reset
        // by the next batch's first fetch, so the last progress event stays
        // valid while idle.
        /** @type {Array<(state: any) => void>} */
        this._loadProgressHooks = [];
        /** @type {Array<(state: any) => void>} */
        this._assetsLoadedHooks = [];
        this._loadBatchTotal = 0;
        this._loadBatchLoaded = 0;
    ...
    _initThreeJS() {
        ...
        // floor-plane intersection. Uses the *framing* bounds, not the
        // near/far content sphere (issue #166): `updateSceneBounds` never
        // filters on visibility on purpose (a hidden object may be shown
        // later and must not get clipped), so a hidden mesh far from the
        // model would drag the pivot off-screen. `_collectFrameableBounds`
        // bails on `.visible === false` subtrees and already skips the
        ...
        // from JS via enableMoveGizmo). Inert until enabled. Its helpers live in
        // their own overlay scene, drawn after the main render over a cleared
        // depth buffer: depth-tested among themselves, on top of everything else.
    ...
    _initClipping() {
        ...
        // same _syncClipFromGizmo handler.
    ...
    /**
     * Sync every orbit-mode indicator with `_orbitMode`: the viewer menu
     * entry and the stack button left of the gimbal (glyph, `.free` accent,
     * tooltip). Called from `_setOrbitMode`, so the R key, either control and
    ...
    _createMaterial(params) {
        ...
        // Front-only culls a translucent body's far walls, leaving a flat
        // silhouette in one shade; opaque bodies hide theirs anyway.
        ...
        // Same rule as applyOpacity, so both paths agree: a translucent body
        // that writes depth occludes its neighbours by draw order (issue #207).
        const { fields: depth, depthWriteExplicit } = depthMaterialFields(params, opacity >= 1);
        /** @type {any} */
        const common = { color, opacity, transparent, wireframe, side, clippingPlanes: clip, ...depth };

        /** @type {any} */ let mat;
        switch (materialType) {
            case 'basic': mat = new THREE.MeshBasicMaterial(common); break;
            case 'phong': mat = new THREE.MeshPhongMaterial(common); break;
            case 'lambert': mat = new THREE.MeshLambertMaterial(common); break;
            default:
                mat = new THREE.MeshStandardMaterial({
                    ...common,
                    roughness: params.roughness != null ? params.roughness : 0.7,
                    metalness: params.metalness != null ? params.metalness : 0.3,
                });
        }
        if (depthWriteExplicit) mat.userData.__depthWriteExplicit = true;
        return mat;
    }
    ...
    _applyTransform(obj, transform) {
        if (!transform) return;
        if (transform.matrix) {
            // Same guard as the animation path: a non-finite element would be
            // decomposed into NaN position/quaternion/scale and the object
            // would vanish for good. Hold the last good pose instead.
            if (!matrixElementsFinite(transform.matrix, 0)) {
                warnNonFiniteMatrix(obj, 'update_transform');
                return;
            }
            obj.matrix.fromArray(transform.matrix);
            obj.matrix.decompose(obj.position, obj.quaternion, obj.scale);
        } else {
            for (const key of ['position', 'rotation', 'quaternion', 'scale']) {
                const v = transform[key];
                if (v && !arrayFinite(v)) {
                    warnNonFiniteMatrix(obj, `update_transform (${key})`);
                    return;
                }
            }
    ...
    _setClipTime(id, time) {
        ...
        if (!mixer) return;
        driveMixerTime(mixer, time);
    }

    /** Normalized clip seek: `t` in [0,1] maps to `t * duration` per clip.
     * @param {string} id @param {number} t */
    _setClipProgress(id, t) {
        const mixer = this._mixers.get(id);
        if (!mixer) {
            if (!this._clipProgressWarned.has(id)) {
                this._clipProgressWarned.add(id);
                console.warn(`set_clip_progress: '${id}' has no animation clips`);
            }
            return;
        }
        driveMixerProgress(mixer, t);
    }
     ...
     * local transform and drive the bound mixer — called once per render
     * frame from `_animate()`. Silently no-ops a binding whose target has no
     * mixer yet or whose source isn't in the scene (either can arrive after
     * the binding, e.g. on a cell load race); it just resolves once both
     * exist. */
    _resolveClipBindings() {
        if (!this._clipBindings.size) return;
        for (const [id, binding] of this._clipBindings) {
            const mixer = this._mixers.get(id);
            const src = this._objects.get(binding.source.id);
            if (!mixer || !src) continue;
            const raw = readLocalChannel(src, binding.source.channel);
    ...
    _fetchPointsLodNode(group, i) {
        const lod = group.userData.pointsLOD;
        const cloudId = group.userData.id;
        lod.loading.add(i);
        // Node payloads live in the same per-cloud blob store Python drops on
        // delete/clear, so these stream fetches cancel with the cloud (#157).
        const abortCtl = this._trackFetch(cloudId);
        (async () => {
            try {
                const buffer = await fetchArrayBuffer(
                    lod.urlBase + i, `points LOD node ${i}`, abortCtl.signal);
                ...
                // The cloud going away mid-stream is expected, not a fault.
                if ((e && e.name === 'AbortError')
                    || this._objects.get(cloudId) !== group) {
                    console.log(`Discarding superseded LOD node ${i} fetch for '${cloudId}'`);
                } else {
                    console.warn(`points LOD node ${i} fetch failed:`, e);
                }
            } finally {
                this._untrackFetch(cloudId, abortCtl);
    ...
    /** Current load token for `id`, 0 when the id was never loaded. @param {string} id */
    _loadTokenOf(id) {
        return this._loadTokens.get(id) || 0;
    }

    /**
     * Register a cancellable blob fetch for `id` and return its
     * AbortController (issue #157). Pass `supersede: true` from a binary
     * *add* case: the newer add owns the id, so any fetch still running for
    ...
    /** @param {string|symbol} id @param {AbortController} controller */
    _untrackFetch(id, controller) {
    ...
    /**
     * True when a failed binary load is the expected fallout of the object
     * being deleted, cleared, or re-pushed rather than a genuine blob miss.
     * `token` is compared only for the fetch stage (installing an object
     * bumps its own token via _deleteObject, so a post-fetch failure must
     * not be excused by it).
     * @param {any} err
     * @param {{id?: string|null, token?: number, scene?: number, anim?: number}} ctx
     */
    _isLoadSuperseded(err, ctx) {
        if (err && err.name === 'AbortError') return true;
        if (ctx.scene !== undefined && this._sceneGeneration !== ctx.scene) return true;
        if (ctx.anim !== undefined && this._animGeneration !== ctx.anim) return true;
        if (ctx.id != null && ctx.token !== undefined
            && this._loadTokenOf(ctx.id) !== ctx.token) return true;
        return false;
    }

    /**
     * Report a failed binary load (issue #157). A load that was deleted or
     * superseded is expected noise and logs one quiet line; anything else
     * runs `loud`, which keeps the fully-attributed console.error of issues
     * #142/#146. Fetch-stage failures that still look live are re-checked
     * after a grace window first, because the 404 (HTTP) can arrive before
     * the delete/re-add that explains it (WebSocket).
     * @param {any} err
     * @param {string} what - subject for the quiet line, e.g. 'mesh'
     * @param {() => void} loud - emits the full console.error
     * @param {{id?: string|null, token?: number, scene?: number, anim?: number,
     *          fetchStage?: boolean}} ctx
     */
    _reportLoadFailure(err, what, loud, ctx) {
        const id = ctx.id;
        const quiet = () => console.log(
            `Discarding superseded ${what} fetch${id != null ? ` for '${id}'` : ''}`);
        if (this._isLoadSuperseded(err, ctx)) { quiet(); return; }
        if (ctx.fetchStage === false) { loud(); return; }
        setTimeout(() => {
            if (this._isLoadSuperseded(err, ctx)) quiet();
            else loud();
        }, supersededFetchGraceMs());
    }
        ...
        if (frame.transforms) {
            ...
            for (const [id, matrix] of Object.entries(frame.transforms)) {
                ...
                if (nextMat) {
                    ...
                    setObjectMatrix4Checked(obj, _lerpMatOut, 'frame transforms');
                } else {
                    setObjectMatrixChecked(obj, matrix, 0, 'frame transforms');
                }
            }
        }
        if (frame.colors) {
    ...
    _maybeNotifyAssetsLoaded() {
        if (this._pendingFetches !== 0 || !this._assetsComplete) return;
        // Deliberately NOT deduplicated, on either path.
        //
        // The WS message is a *reply*, not an event: wait_for_assets() clears
        // its event, sends mark_assets_complete, and blocks until this lands.
        // Suppressing a repeat because "nothing changed since the last one"
        // would hang the second wait_for_assets(disconnect=False) in a script
        // that stages its scene in two passes.
        //
        // The hooks then fire whenever the viewer is genuinely idle with the
        // producer's stream marked complete, which is what an embedder's
    ...
    _onFetchStart() {
        if (this._pendingFetches === 0) {
            // New batch: clear the retained counters of the previous one.
            this._loadBatchTotal = 0;
            this._loadBatchLoaded = 0;
        }
        this._pendingFetches++;
        this._loadBatchTotal++;
        this._fireLoadProgress();
    }
    _onFetchEnd() {
        this._pendingFetches = Math.max(0, this._pendingFetches - 1);
        this._loadBatchLoaded = Math.min(this._loadBatchTotal, this._loadBatchLoaded + 1);
        this._fireLoadProgress();
        this._maybeNotifyAssetsLoaded();
    }

    /**
     * Snapshot of the viewer's asset-fetch state — the public replacement
     * for polling the private `_pendingFetches` counter.
     * @returns {{loaded: number, total: number, pending: number, assetsComplete: boolean}}
     */
    getLoadState() {
        return {
            loaded: this._loadBatchLoaded,
            total: this._loadBatchTotal,
            pending: this._pendingFetches,
            assetsComplete: this._assetsComplete,
        };
    }

    /** Fire the onLoadProgress hooks (no-op with none registered). */
    _fireLoadProgress() {
        if (!this._loadProgressHooks.length) return;
        const state = this.getLoadState();
        for (const cb of this._loadProgressHooks.slice()) {
            try { cb(state); } catch (err) { console.error('onLoadProgress hook error', err); }
        }
    }

    /** Fire the onAssetsLoaded hooks (no-op with none registered). */
    _fireAssetsLoaded() {
        if (!this._assetsLoadedHooks.length) return;
        const state = this.getLoadState();
        for (const cb of this._assetsLoadedHooks.slice()) {
            try { cb(state); } catch (err) { console.error('onAssetsLoaded hook error', err); }
        }
    }
        ...
        switch (data.type) {
            ...
            case 'query_scene': {
                ...
                for (const [id, obj] of this._objects) {
                    ...
                    // Additive: models with embedded animation clips report
                    // their per-clip durations (seconds) so consumers can
                    // discover animated ids for set_clip_time/progress.
                    const mixer = this._mixers.get(id);
                    if (mixer) tree[id].clipDurations = mixerClipDurations(mixer);
                }
                return this._reply({
            ...
            case 'load_animation_http': {
                ...
                // Python drops the previous animation blob as it sends the new
                // one, so a swap must cancel the old fetch (issue #157).
                const abortCtl = this._trackFetch(ANIMATION_FETCH_KEY, { supersede: true });
                (async () => {
                    let fetched = false;
                    try {
                        const t0 = performance.now();
                        const buffer = await fetchArrayBuffer(
                            data.blob_url, 'load_animation_http', abortCtl.signal);
                        fetched = true;
                        ...
                        this._reportLoadFailure(e, 'animation',
                            () => console.error('Error loading animation via HTTP:', e),
                            { scene: capturedScene, anim: capturedAnim, fetchStage: !fetched });
                    } finally {
                        this._untrackFetch(ANIMATION_FETCH_KEY, abortCtl);
                        this._onFetchEnd();
                    }
                })();
                break;
            ...
            case 'add_polyline_binary': {
                ...
                (async () => {
                    ...
                    try {
                        const buffer = await fetchArrayBuffer(
                            data.blob_url, `add_polyline_binary '${data.id}'`, abortCtl.signal);
                        fetched = true;
                        if (this._sceneGeneration !== capturedScene) {
            ...
            case 'add_points_binary': {
                ...
                (async () => {
                    ...
                    try {
                        const buffer = await fetchArrayBuffer(
                            data.blob_url, `add_points_binary '${data.id}'`, abortCtl.signal);
                        fetched = true;
                        if (this._sceneGeneration !== capturedScene) {
                        ...
                        this._reportLoadFailure(e, 'point cloud',
            ...
            case 'add_points_lod': {
                ...
                (async () => {
                    ...
                    try {
                        ...
                        this._reportLoadFailure(e, 'LOD point cloud',
                ...
                this._withObject(data.id, 'update_polyline_colors', (target) => {
                    ...
                    (async () => {
                        ...
                        try {
                            ...
                            this._reportLoadFailure(e, 'polyline color',
            ...
            case 'add_mesh_binary': {
                ...
                (async () => {
                    ...
                    try {
                        const buffer = await fetchArrayBuffer(
                            data.blob_url, `add_mesh_binary '${data.id}'`, abortCtl.signal);
                        ...
                        // Per-vertex alpha is interleaved RGBA via vertexColorComponents: 4;
                        // a planar alpha tail flagged hasVertexAlpha is a different layout.
                        if (data.hasVertexAlpha && vcc !== 4 && !warnedVertexAlphaFlag) {
                            warnedVertexAlphaFlag = true;
                            console.warn('add_mesh_binary: hasVertexAlpha is ignored; send interleaved RGBA with vertexColorComponents: 4');
                        }
                        ...
                        const { fields: depth, depthWriteExplicit } = depthMaterialFields(data, !isTransparent);
                        ...
                        if (depthWriteExplicit) meshMaterial.userData.__depthWriteExplicit = true;
                        ...
                        const got = byteLength >= 0
                            ...
                            : '';
                        this._reportLoadFailure(e, 'mesh',
                            () => console.error(`Error creating mesh '${data.id}'${got}:`, e),
                            { id: data.id, token: byteLength >= 0 ? undefined : loadToken,
                              scene: capturedScene, fetchStage: byteLength < 0 });
                        deferred.reject(e);
                    } finally {
                        this._untrackFetch(data.id, abortCtl);
                        if (this._inflightLoads.get(data.id) === deferred) {
            ...
            case 'add_parametric_tube_binary': {
                ...
                (async () => {
                    ...
                    try {
                        ...
                        this._reportLoadFailure(e, 'parametric tube',
            ...
            case 'add_swept_tool_binary': {
                ...
                (async () => {
                    ...
                    try {
                        const buffer = await fetchArrayBuffer(
                            data.blob_url, `add_swept_tool_binary '${data.id}'`, abortCtl.signal);
                        fetched = true;
                        if (this._sceneGeneration !== capturedScene) {
                        ...
                        this._reportLoadFailure(e, 'swept tool',
                ...
                this._withObject(data.id, 'update_parametric_tube_colors', (target) => {
                    ...
                    (async () => {
                        ...
                        try {
                            ...
                            this._reportLoadFailure(e, 'parametric tube color',
            ...
            case 'update_axis_control':
                this._axisControls.setValue(data.id, {
                    value: data.value, min: data.min, max: data.max,
                    color: data.color, hoverColor: data.hover_color,
                });
                break;
            ...
            case 'set_follow_path': {
                ...
                (async () => {
                    ...
                    try {
                        const buffer = await fetchArrayBuffer(
                            data.blob_url, `set_follow_path '${data.id}'`, abortCtl.signal);
                        fetched = true;
                        ...
                        this._reportLoadFailure(e, 'follow path',
            ...
            case 'set_move_gizmo':
                ...
                break;
            case 'set_gizmo_axes':
                this._transformGizmo.setAxes({ x: data.x, y: data.y, z: data.z, translate: data.translate, rotate: data.rotate });
                break;
            case 'add_gizmo': {
                ...
                if (target) this._transformGizmo.addGizmo(target, {
                    id: data.id,
                    mode: data.mode,
                    axes: { x: data.x, y: data.y, z: data.z, translate: data.translate, rotate: data.rotate },
                    space: data.space,
                    snapDefault: data.snapDefault,
                    color: data.color,
                    hoverColor: data.hoverColor,
            ...
            case 'set_billboard':
                // Orient-to-viewer as a per-object property. Transient like
                // set_color/set_highlight — not replayed on reconnect.
                this._withObject(data.id, 'set_billboard', (obj) => {
                    if (data.enabled === false) this._disableBillboard(data.id, obj);
                    else this._enableBillboard(data.id, obj, data);
                });
                break;
            case 'add_billboard': {
                ...
                // After _deleteObject, which prunes the map for this id.
    ...
    /**
     * Turn the orient-to-viewer property on (or re-configure it) for one object.
     * @param {string} id
     * @param {any} obj
     * @param {any} data the add_billboard / set_billboard payload
     */
    _enableBillboard(id, obj, data) {
        obj.userData.__isBillboard = true;
        obj.userData.__billboardOpts = resolveBillboardOptions(data, id);
        // Whatever pose the producer has set so far is the base we compose on.
        captureBillboardBase(obj);
    ...
    /**
     * Turn it off and restore the producer's pose exactly.
     * @param {string} id
     * @param {any} obj
     */
    _disableBillboard(id, obj) {
    ...
    /**
     * Billboards ordered parents-first.
     *
     * A child billboard reads its parent's world quaternion, so insertion
     * order would leave it a frame behind whenever the child was registered
     * first — an angular lag on its own, but a *positional* one once a pivot
     * is in play, which visibly swims during an orbit.
     * @returns {any[]}
     */
    _billboardOrder() {
        if (!this._billboardOrderDirty
            && this._billboardOrderGen === this._objGeneration
            && this._billboardSorted) {
            return this._billboardSorted;
        }
        const arr = [...this._billboards.values()];
        for (const obj of arr) {
            let depth = 0;
            for (let p = obj.parent; p; p = p.parent) depth++;
            obj.userData.__billboardDepth = depth;
        }
        arr.sort((a, b) => a.userData.__billboardDepth - b.userData.__billboardDepth);
        this._billboardSorted = arr;
        this._billboardOrderDirty = false;
        this._billboardOrderGen = this._objGeneration;
        return arr;
    }
    ...
    _updateBillboards() {
        ...
        for (const obj of this._billboardOrder()) {
            if (!obj.visible || !obj.parent) continue;
            // A gizmo drag owns the transform for its duration: re-aiming here
            // would fight the drag and report a camera-derived pose back to
            // the producer as if the user had chosen it.
            if (obj.userData.__billboardDragging) continue;
            applyBillboard(obj, this._camera);
        }
    }
    ...
    _animate() {
        ...
        // Live jog/drag clip bindings (issue #225) — unaffected by, and runs
        // regardless of, recorded-animation scrubbing above (that path drives
        // clip times from `frame.clip_times` instead).
        this._resolveClipBindings();
        ...
        if (this._clipGizmo.enabled) {
            restyleGizmoHelper(this._clipGizmoHelper);
            restyleGizmoHelper(this._clipMoveGizmoHelper);
        }
     ...
     * (issue #163) — lets an embedder drive a loading overlay without
     * polling the private `_pendingFetches` counter.
     *
     * Semantics: binary payloads are fetched as their messages arrive, so
     * there is no honest up-front total. `total` is the number of fetches
     * *started* in the current load batch (a batch opens on the first fetch
     * while idle and closes when the last one finishes) and may grow while
     * the batch is in flight; `loaded` is how many of them have finished.
     * `pending === 0` means every fetch the viewer knows about is done —
     * the honest "spinner off" signal; it does not promise the producer has
     * no more messages to send (that is `onAssetsLoaded`). The finished
     * batch's counters are retained while idle and reset by the next batch.
     * @param {(state: {loaded:number, total:number, pending:number, assetsComplete:boolean}) => void} cb
     * @returns {() => void} unsubscribe
     */
    onLoadProgress(cb) {
        this._loadProgressHooks.push(cb);
        return () => {
            const i = this._loadProgressHooks.indexOf(cb);
            if (i >= 0) this._loadProgressHooks.splice(i, 1);
        };
    }

    /**
     * Register a hook fired when the viewer goes fully idle: no fetch is in
     * flight *and* the producer has declared its stream complete by sending
     * `{type: 'mark_assets_complete'}` (Python's `wait_for_assets()` sends
     * it; a no-WebSocket embedder feeds it through `handleMessage()` after
     * its last add). This is the browser side of the existing
     * `assets_loaded` WS notification, and now fires on the direct
     * `handleMessage()` path too — no socket required. Without the
     * completion marker it never fires; watch `onLoadProgress`'s
     * `pending === 0` for the weaker "everything requested so far has
     * landed". May fire again if further assets arrive after a completion.
     * @param {(state: {loaded:number, total:number, pending:number, assetsComplete:boolean}) => void} cb
     * @returns {() => void} unsubscribe
     */
    onAssetsLoaded(cb) {
        this._assetsLoadedHooks.push(cb);
        return () => {
            const i = this._assetsLoadedHooks.indexOf(cb);
            if (i >= 0) this._assetsLoadedHooks.splice(i, 1);
        };
    }
    ...
    /** Direct-call JS equivalent of the `add_axis_control` WS message — same
     * effect, for an embedder holding a live `viewer` reference in the same
     * page (no WS round trip needed). `opts`: `{targetId, axis, kind, value,
     * min, max, color, hoverColor, radius, bboxSourceId, window}` — see the
     * WS message for the full shape. `hoverColor` is optional; omitting it
     * auto-derives a lightened variant of `color`. Values are plain
     * THREE.js scene units: metres for linear(_unlimited), radians for
     * rotary(_unlimited) — no CSD-unit conversion happens here.
     * @param {string} id @param {any} opts */
    addAxisControl(id, opts) { this._axisControls.add(id, opts); }

    /** @param {string} id @param {{value?:number, min?:number, max?:number, color?:number|string|null, hoverColor?:number|string|null}} opts */
    updateAxisControl(id, opts) { this._axisControls.setValue(id, opts); }
     ...
     * @param {{mode?:string, axes?:GizmoAxesMask|null, id?:string|null, space?:string, snapDefault?:boolean, color?:number|string|null, hoverColor?:number|string|null, scale?:number}} [opts]
    ...
    addGizmo(objectOrId, opts = {}) {
        ...
        if (!obj) return null;
        return this._transformGizmo.addGizmo(obj, {
            mode: opts.mode, axes: opts.axes, id, space: opts.space, snapDefault: opts.snapDefault,
            color: opts.color, hoverColor: opts.hoverColor, scale: opts.scale,
        });
    }
     ...
     * that axis; omitted keys (or `setGizmoAxes(null)`) show all axes. Top-level
     * `x`/`y`/`z` cover both modes; a `translate` or `rotate` sub-mask replaces
     * them for that mode, e.g. `{rotate: {z: false}}` keeps every arrow but hides
     * the Z ring. Resets to all-axes automatically when the gizmo detaches.
     * @param {GizmoAxesMask|null} [mask]
     */
