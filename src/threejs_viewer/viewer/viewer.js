...
// closest-pair midpoint between segment(p_a, p_{a+1}) and segment(p_b, p_{b+1})
// is recorded as the per-detection fold target along with the squared
// closest-pair gap. Overlapping detection ranges merge; each merged range
// snaps every ring inside it to the midpoint of the *tightest* constituent
// detection (the one whose two segments came closest to each other).
//
// Why "tightest" instead of average: the closest-pair midpoint of two
// near-crossing segments lies on the perpendicular bisector of the closest
// pair of points — which approaches the strand's actual self-intersection
// only as the gap shrinks to zero. Long-range detections (segment pairs far
// apart along the strand but still inside the window) have closest-pair
// midpoints biased toward the medial axis of the fold's interior, away
// from the apex. Averaging short-range and long-range midpoints together
// drags the snap point toward the centroid of that interior cloud. Picking
// the single detection with the smallest closest-pair gap lands the snap
// at the actual crossing apex regardless of how many other looser
// detections fired in the same range.
//
...
// close to an earlier segment but not vice versa. The scan walks every k in
// the window per direction (no early-exit), tracking the largest-k hit (for
// range coverage) and the tightest-gap hit (for the snap midpoint) in one
// pass. Cost is O(nCs · N · W); first-hit-from-furthest early-exit was
// dropped because it systematically picked the loose pair at gap ≈ tol —
// for deep folds the tightest pair is at smaller k, which the early-exit
// scan never visited. Per ring per direction at most one synthesised
// detection is pushed (largest range + tightest midpoint, decoupled).
//
...
function collapseTubeStrandFolds(positions, widths, heights, nSpine, nCs) {
    ...
    for (let j = 0; j < nCs; j++) {
        ...
        for (let i = 0; i < nSpine; i++) {
            ...
            // Forward scan: walk every k in [i+2, i+W] — no early-exit. We
            // need both the largest k that detects (for range coverage) and
            // the tightest detection in the window (for the snap midpoint).
            // First-hit-from-furthest would systematically pick a loose pair
            // at gap ≈ tol; the deeper fold's tightest pair (typically at a
            // smaller k inside the window) would be skipped.
            const kHi = Math.min(nSpine - 2, i + window);
            let kLargestF = -1;
            let bestD2F = Infinity, bestXF = 0, bestYF = 0, bestZF = 0;
            for (let k = i + 2; k <= kHi; k++) {
                ...
                if (dx * dx + dy * dy + dz * dz < tolSq) {
                    if (k > kLargestF) kLargestF = k;
                    // a-segment for forward detection: (p_i, p_{i+1}). Always
                    // valid since k ≤ nSpine-2 and k > i imply i+1 ≤ nSpine-2.
                    strandSegSegMidpoint(positions, ringStride, j, i, k, foldOut);
                    if (foldOut[3] < bestD2F) {
                        bestD2F = foldOut[3];
                        bestXF = foldOut[0];
                        bestYF = foldOut[1];
                        bestZF = foldOut[2];
            ...
            if (kLargestF >= 0) {
                rangeStart.push(i);
                rangeEnd.push(kLargestF + 1);
                foldX.push(bestXF);
                foldY.push(bestYF);
                foldZ.push(bestZF);
                foldD2.push(bestD2F);
            }
            // Backward scan: walk every k in [i-W, i-2].
            const kLo = Math.max(0, i - window);
            let kSmallestB = nSpine;
            let bestD2B = Infinity, bestXB = 0, bestYB = 0, bestZB = 0;
            for (let k = kLo; k <= i - 2; k++) {
                ...
                const dz = pz - (az + t * ez);
                if (dx * dx + dy * dy + dz * dz < tolSq) {
                    if (k < kSmallestB) kSmallestB = k;
                    // a-segment for backward detection: (p_{i-1}, p_i). The
                    // backward inner-loop condition k ≤ i-2 forces i ≥ 2, so
                    // i-1 ≥ 1 — always valid.
                    strandSegSegMidpoint(positions, ringStride, j, i - 1, k, foldOut);
                    if (foldOut[3] < bestD2B) {
                        bestD2B = foldOut[3];
                        bestXB = foldOut[0];
                        bestYB = foldOut[1];
                        bestZB = foldOut[2];
            ...
            if (kSmallestB < nSpine) {
                rangeStart.push(kSmallestB);
                rangeEnd.push(i);
                foldX.push(bestXB);
                foldY.push(bestYB);
                foldZ.push(bestZB);
        ...
        const _d2 = foldD2.slice();
        Array.prototype.sort.call(order, (a, b) => _starts[a] - _starts[b]);
        // Merge overlapping ranges in order. Within each merged range keep
        // only the tightest detection's midpoint (smallest closest-pair gap²).
        const o0 = order[0];
