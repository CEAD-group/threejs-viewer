...
// `baseNext` given the current (baseNext, t, channel-metadata) state.
// Channels can opt out via `ch.interpolation === "step"`; integer dtypes and
...
function makeChannelApply(viewer) {
    return {
        ...
        colors(ch, refs, base) {
            // Always step: indexed or hex values aren't meaningful to lerp.
            if (ch._objGen !== viewer._objGeneration) {
                refreshRefs(refs, ch.ids, viewer._objects);
                ch._objGen = viewer._objGeneration;
            }
        ...
        visibility(ch, refs, base) {
            // Always step: uint8, discrete by nature.
            if (ch._objGen !== viewer._objGeneration) {
                refreshRefs(refs, ch.ids, viewer._objects);
                ch._objGen = viewer._objGeneration;
            }
        ...
        clip_times(ch, refs, base, baseNext, t) {
            ...
            const nObj = ch.ids.length;
            // Default step (clip timelines are user-facing seek values); opt-in
            // linear via metadata={"interpolation": "linear"} on the channel.
            const interp = baseNext !== null && t > 0 && ch.interpolation === 'linear';
            for (let i = 0; i < nObj; i++) {
                const mixer = refs[i];
...
export class ThreeJSViewer {
    ...
    _loadAnimation(animData) {
        ...
        this._animation.interpolation = animData.interpolation === 'linear' ? 'linear' : 'step';
        this._animationTime = 0;
        this._animationPlaying = false;
    ...
    _applyFrame(frameIndex, t = 0) {
        ...
        // Global interpolation mode — set per-animation via animation.interpolation.
        // Short-circuit to step when: mode is "step", t is effectively zero/one,
        // or we're at the last keyframe (no next frame to interpolate to).
        const interpGlobal = this._animation.interpolation === 'linear'
            && t > 1e-6 && t < 1 - 1e-6
            && frameIndex < frames.length - 1;
        const nextFrame = interpGlobal ? frames[frameIndex + 1] : null;
        ...
        if (this._animation.channels) {
            ...
            for (const [name, ch] of Object.entries(this._animation.channels)) {
                ...
                if (applyFn) {
                    ...
                    const baseNext = interpGlobal ? (frameIndex + 1) * nObj * ch.stride : null;
                    applyFn(ch, ch.refs, base, baseNext, interpGlobal ? t : 0);
                }
            }
        }

        if (frame.transforms) {
            const nextT = (nextFrame && nextFrame.transforms) ? nextFrame.transforms : null;
        ...
        if (frame.colors) {
            // Step: color interpolation isn't meaningful for hex-coded values
            // (mode switches between discrete colorings should land exactly).
        ...
        if (frame.opacity) {
            const nextO = (nextFrame && nextFrame.opacity) ? nextFrame.opacity : null;
        ...
        if (frame.clip_times) {
            // Step: clip times seek into embedded GLTF mixer clocks; the mixer
            // has its own keyframe structure and stepping avoids compounding.
            for (const [id, time] of Object.entries(frame.clip_times)) {
                this._setClipTime(id, time);
            }
        }
        if (frame.draw_ranges) {
            const nextD = (nextFrame && nextFrame.draw_ranges) ? nextFrame.draw_ranges : null;
        ...
        this._applyCameraTracking(frameIndex, interpGlobal ? (frameIndex + 1) : frameIndex, interpGlobal ? t : 0);
    }
        ...
        if (this._trackMode === 'scripted' && this._animation?.channels) {
            ...
            if (ctCh || cpCh) {
                ...
                if (ctCh) {
                    ...
                    if (interp && ctCh.interpolation !== 'step') {
                ...
                if (cpCh) {
                    const base = frameIndex * 3;
                    if (interp && cpCh.interpolation !== 'step') {
                        const bN = frameIndexNext * 3;
