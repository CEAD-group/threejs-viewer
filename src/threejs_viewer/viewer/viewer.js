...
// strand j, segment b similarly). Writes the midpoint into out[0..2]. Uses
// the standard parametric-clamp formulation (Real-Time Collision Detection,
// Ericson, §5.1.9): solve the unconstrained 2D minimum of |P(s)-Q(t)|², then
// clamp s, t to [0,1] in the order that respects the constraint normals.
// Degenerate-segment branches kept for completeness; in practice both
// segments are non-degenerate strand pieces.
/**
...
// is recorded as the per-detection fold target. Overlapping detection ranges
// merge; each merged range snaps every ring inside it to the AVERAGE of its
// constituent fold targets.
//
// Why segment-segment closest-pair midpoint (not foot-of-vertex midpoint):
// the foot of p_i on seg(k, k+1) is, by detection, within `tol` of p_i, so
// (p_i + foot)/2 ≈ p_i — averaging those midpoints across a range reduces
// to the centroid of ring positions in that range, which drags the snap
// target toward the spine when the fold spans many rings. The closest-pair
// midpoint instead sits in the *gap* between the two folding segments
// (typically near the geometric crossing line of the fold), so averaging it
// across detections keeps the snap target at the geometric fold location
// rather than drifting toward the spine.
//
