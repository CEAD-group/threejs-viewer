...
const LOD_CHUNK_REUSE_RATIO = 1.5;  // reuse chunk if distance ratio within [1/R, R]
...
// RDP on a single chunk range [chunkStart, chunkEnd]. Returns array of kept
// indices (sorted), including the endpoints.
function rdpChunk(spine, epsSq, chunkStart, chunkEnd) {
    const keep = [];
    keep.push(chunkStart);
...
// Per-tube chunk cache: tubeId -> { chunkDists: Float64Array, chunkIndices: Array<number[]> }
const _cache = new Map();

self.onmessage = function(e) {
    const { tubeId, spine, nPoints, camX, camY, camZ } = e.data;
    if (nPoints <= 2) {
        const indices = new Uint32Array(nPoints < 1 ? 0 : nPoints === 1 ? [0] : [0, 1]);
        self.postMessage({ tubeId, indices, minDist: 0, maxDist: 0, chunksReused: 0, chunksTotal: 0 }, [indices.buffer]);
        return;
    }

    // Pre-compute per-point epsilon² and distance stats
    ...
    let cached = _cache.get(tubeId);
    if (!cached || cached.chunkDists.length !== nChunks) {
        cached = { chunkDists: new Float64Array(nChunks), chunkIndices: new Array(nChunks) };
        _cache.set(tubeId, cached);
    }

    // Per-chunk: compute center-point distance, decide reuse or recompute
    let totalKept = 0;
    let chunksReused = 0;
    for (let ci = 0; ci < nChunks; ci++) {
        const chunkStart = ci * LOD_CHUNK_SIZE;
        const chunkEnd = Math.min(chunkStart + LOD_CHUNK_SIZE, nPoints - 1);
        // Distance from camera to chunk midpoint
        const mid = Math.floor((chunkStart + chunkEnd) / 2);
        const dx = spine[mid * 3] - camX;
        const dy = spine[mid * 3 + 1] - camY;
        const dz = spine[mid * 3 + 2] - camZ;
        const dist = Math.sqrt(dx * dx + dy * dy + dz * dz);

        const prevDist = cached.chunkDists[ci];
        if (prevDist > 0 && cached.chunkIndices[ci]) {
        ...
        const kept = rdpChunk(spine, epsSq, chunkStart, chunkEnd);
        cached.chunkDists[ci] = dist;
        cached.chunkIndices[ci] = kept;
        totalKept += kept.length;
    }

    // Merge all chunk indices (already sorted within each chunk, chunks are in order)
    // Deduplicate boundary points shared between adjacent chunks
    const indices = new Uint32Array(totalKept);
    let j = 0;
    for (let ci = 0; ci < nChunks; ci++) {
        const kept = cached.chunkIndices[ci];
        for (let k = 0; k < kept.length; k++) {
            if (j > 0 && kept[k] === indices[j - 1]) continue;  // skip duplicate boundary
            indices[j++] = kept[k];
        }
    }
    const result = indices.subarray(0, j);
    // Copy to transferable buffer
    const out = new Uint32Array(result);
    self.postMessage({ tubeId, indices: out, minDist, maxDist, chunksReused, chunksTotal: nChunks }, [out.buffer]);
};
`;
...
// Rebuild a parametric tube mesh with a reduced spine (from LOD).
// keptIndices: Uint32Array of indices into the original spine arrays.
function rebuildTubeLOD(mesh, keptIndices) {
    const lod = mesh.userData.tubeLOD;
    const nRed = keptIndices.length;
    if (nRed < 2) return;

    // Extract reduced arrays from originals
    const reducedSpine = new Float32Array(nRed * 3);
    const reducedWidths = new Float32Array(nRed);
    const reducedHeights = new Float32Array(nRed);
    let reducedRingColors = null;
    if (lod.originalRingColors) {
        reducedRingColors = new Float32Array(nRed * 3);
    }
    for (let i = 0; i < nRed; i++) {
        const oi = keptIndices[i];
        reducedSpine[i * 3] = lod.originalSpine[oi * 3];
        reducedSpine[i * 3 + 1] = lod.originalSpine[oi * 3 + 1];
        reducedSpine[i * 3 + 2] = lod.originalSpine[oi * 3 + 2];
        reducedWidths[i] = lod.originalWidths[oi];
        reducedHeights[i] = lod.originalHeights[oi];
        if (reducedRingColors) {
            reducedRingColors[i * 3] = lod.originalRingColors[oi * 3];
            reducedRingColors[i * 3 + 1] = lod.originalRingColors[oi * 3 + 1];
            reducedRingColors[i * 3 + 2] = lod.originalRingColors[oi * 3 + 2];
        }
    }

    // Build new geometry
    const result = buildParametricTubeGeometry(
        reducedSpine, reducedWidths, reducedHeights,
        null, lod.upVector, reducedRingColors,
    );
    ...
    const nCs = result.nCs;
    ud.tubeNumSpinePoints = nRed;
    ud.tubeRingPairs = result.ringPairs;
    ud.tubeIndicesPerRingPair = result.indicesPerRingPair;
    ud.totalIndexCount = result.capIndicesPerCap + result.ringPairs * result.indicesPerRingPair + result.capIndicesPerCap;
    ud.tubeCapIndicesPerCap = result.capIndicesPerCap;
    ud.tubeEndCapBase = result.endCapBase;

    // Rebuild morphData for frontier morphing
    ud.tubeMorphData = {
        spine: reducedSpine,
        widths: reducedWidths,
        heights: reducedHeights,
        localFrames: result.localFrames,
        capAngles: result.capAngles,
        ringColors: reducedRingColors,
        section: new Float32Array(nCs * 2),
        savedRing: new Float32Array(nCs * 3),
        savedRingColors: null,
        savedRingIndex: null,
        morphedState: null,
        endCapPattern: result.endCapPattern,
        savedCapIndices: new result.endCapPattern.constructor(result.endCapPattern.length),
        savedCapOffset: -1,
    };

    // Update LOD state
    lod.keptIndices = keptIndices;

    // Re-apply draw range with arc-length remapping
    applyParametricTubeDrawRange(mesh, lod.currentDrawRange);
}
...
function applyParametricTubeDrawRange(obj, value) {
    ...
    // Track draw_range for LOD rebuilds
    if (ud.tubeLOD) ud.tubeLOD.currentDrawRange = value;

    // Remap when LOD is active: preserve original point-index pacing
    let fracRingPairs;
...
export class ThreeJSViewer {
    ...
    _initThreeJS() {
        ...
        this._lodWorker.onmessage = (e) => {
            const { tubeId, indices, minDist, maxDist, chunksReused, chunksTotal } = e.data;
            this._lodWorkerBusy = false;
            const obj = this._objects.get(tubeId);
            if (!obj || !obj.userData.tubeLOD) return;
            // Skip geometry rebuild if all chunks were reused (indices unchanged)
            if (chunksReused === chunksTotal) {
                console.log(`LOD ${tubeId}: ${indices.length} pts (unchanged, ${chunksTotal} chunks reused)`);
            } else {
                const t0 = performance.now();
                rebuildTubeLOD(obj, indices);
                console.log(`LOD ${tubeId}: ${obj.userData.tubeLOD.originalCount} → ${indices.length} pts (rebuild ${(performance.now() - t0).toFixed(1)}ms) chunks ${chunksReused}/${chunksTotal} reused, dist=[${minDist.toFixed(1)}, ${maxDist.toFixed(1)}]`);
            }
            // Re-trigger is handled by the normal _animate throttle — don't bypass it
        };
                        ...
                        (async () => {
                            try {
                                ...
                                if (n >= 200) {
                                    ...
                                    console.log(`LOD initial ${data.id}: ${n} → ${nRed} pts`);
                                }
                                ...
                                if (tubeLOD) mesh.userData.tubeLOD = tubeLOD;
                                this._addToParentOrScene(mesh, data.parent);
                                this._objects.set(data.id, mesh);
                                this._objGeneration++;
                                if (data.transform) this._applyTransform(mesh, data.transform);
                        ...
                        (async () => {
                            try {
                                ...
                                if (lod && lod.keptIndices) {
                                    ...
                                    if (lod) lod.originalRingColors = new Float32Array(rc);
                                    const n = nOrig;
                                    // Fill cap dome vertices with a single color
                                    function fillCapColors(arr, baseVert, capVerts, r, g, b) {
                                        for (let j = 0; j < capVerts; j++) {
    ...
    _dispatchLodWorker() {
        ...
        for (const obj of this._objects.values()) {
            ...
            // Post spine data to worker — spine is a SharedArrayBuffer-like view,
            // so we copy it for transfer to avoid detaching the original.
            this._lodWorkerBusy = true;
            this._lodWorker.postMessage({
                tubeId: obj.name,
                spine: lod.originalSpine,
                nPoints: lod.originalCount,
                camX, camY, camZ,
            });
