...
function buildParametricTubeGeometry(
    ...
    // Constant-up frame derivation: V is the global up vector (default +Z)
    // projected onto the plane perpendicular to the tangent, so "height"
    // means "up" for horizontal beads. U = V × T completes the frame.
    // When the tangent is parallel to up (vertical segments), V falls
    // back to global +X to avoid degeneracy.
    ...
    for (let i = 0; i < nSpine; i++) {
        ...
        writeRingVerts(positions, ringBase, section, nCs,
                       _U.x, _U.y, _U.z, _V.x, _V.y, _V.z, sx, sy, sz, vOff);
        if (colors) {
