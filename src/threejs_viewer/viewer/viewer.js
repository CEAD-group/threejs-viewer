...
// white lines via two controls:
//
//   `D`        cycles a mutually-exclusive BASE appearance:
//                0 off         plain as-authored
//                1 fog         classic CAD depth cueing — far lines dim to black
//                2 width       perspective line width (worldUnits): far thinner
//                3 coolwarm    aerial-perspective tint — far lines fade to blue
//                4 autorotate  structure-from-motion turntable spin
//                5 halo        haloed lines — a dark gap is carved around the
//                              nearer strand at crossings (Appel 1979)
//
//   `Shift+D`  toggles EYE-DOME LIGHTING as an independent overlay: a
//              screen-space post-process (Potree/CloudCompare) that darkens
//              fragments sitting behind their neighbours, sculpting flat-white
//              line bundles into legible 3D. Because it operates on the final
//              rendered colour it COMPOSES with any base mode — e.g. fog+EDL
//              gives global distance dimming plus local surface shading.
//
// Purely viewer-side, applies to every polyline present at toggle time — there
// is no Python API.
    ...
    fragmentShader: /* glsl */ `
        ...
        void main() {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                sum += max(0.0, logC - log2(linearDist(vUv + offs[i]) + 1e-6));
            }
...
// Base appearance modes cycled by `D`. Eye-dome lighting is NOT here — it is an
// independent overlay (Shift+D) that composes with whichever base is active.
// Distant lines dim toward this near-black for classic CAD depth cueing.
...
class DepthCueController {
    ...
    /** Programmatic fog on/off (idempotent). */
    setFog(on) {
        const want = !!on;
    ...
    /** Programmatic EDL on/off (idempotent). */
    setEdl(on) {
        const want = !!on;
    ...
    _updateFogRange() {
        ...
        const dist = this.v._camera.position.distanceTo(center);
    ...
    onResize(width, height) {
        if (this._composer) {
            ...
            const db = this.v._renderer.getDrawingBufferSize(this._tmpV2);
            if (this._depthTexture) {
                this._depthTexture.image.width = db.x;
                this._depthTexture.image.height = db.y;
...
export class ThreeJSViewer {
    ...
    constructor(container, options = /** @type {ThreeJSViewerOptions} */ ({})) {
        ...
        // D-key depth-cue cycle for flat white line drawings (fog / perspective
        // width / cool-warm tint / auto-rotate / haloed lines / eye-dome lighting).
