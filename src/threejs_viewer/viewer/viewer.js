...
function buildGeometry(spine, widths, heights, upVec, ringColors) {
    ...
    for (let i = 0; i < nSpine; i++) {
        const ip = Math.max(i - 1, 0), in_ = Math.min(i + 1, nSpine - 1);
        let tx = spine[in_ * 3] - spine[ip * 3];
        let ty = spine[in_ * 3 + 1] - spine[ip * 3 + 1];
        let tz = spine[in_ * 3 + 2] - spine[ip * 3 + 2];
    ...
    // Frames + vertex positions
    for (let i = 0; i < nSpine; i++) {
        const tx = tangents[i * 3], ty = tangents[i * 3 + 1], tz = tangents[i * 3 + 2];
        // Constant-up frame: project up onto plane perp to tangent
        const dot = tx * ux0 + ty * uy0 + tz * uz0;
        let sx_, sy_, sz_;
        if (Math.abs(dot) > 0.99) { sx_ = fbx; sy_ = fby; sz_ = fbz; }
        else { sx_ = ux0; sy_ = uy0; sz_ = uz0; }
        // V = seed - dot(seed,T)*T, normalized
        const sdot = sx_ * tx + sy_ * ty + sz_ * tz;
        let vx = sx_ - sdot * tx, vy = sy_ - sdot * ty, vz = sz_ - sdot * tz;
    ...
    return { positions, colors, indices, localFrames, capAngles, endCapPattern,
             ringPairs, indicesPerRingPair, capIndicesPerCap, endCapBase,
             nSpine, totalVerts, is32bit: totalVerts > 65535 };
}

// ---- Per-tube state cache ----
const _tubes = new Map();   // tubeId -> { spine, widths, heights, ringColors, upVec, nPoints }
...
self.onmessage = function(e) {
    ...
    const transfer = [geo.positions.buffer, geo.indices.buffer, geo.localFrames.buffer,
    ...
    self.postMessage({
        ...
        positions: geo.positions, colors: geo.colors, indices: geo.indices,
...
// Apply pre-built geometry from LOD worker to a parametric tube mesh.
// msg: worker message containing typed arrays + metadata.
function applyWorkerGeometry(mesh, msg) {
    const lod = mesh.userData.tubeLOD;
    const nRed = msg.nSpine;
    const nCs = N_CROSS_SECTION;

    // Build Three.js geometry from worker arrays
    const geometry = new THREE.BufferGeometry();
...
export class ThreeJSViewer {
    ...
    _initThreeJS() {
        ...
        this._lodWorker.onmessage = (e) => {
            ...
            console.log(`LOD ${msg.tubeId}: ${obj.userData.tubeLOD.originalCount} → ${msg.nSpine} pts (upload ${(performance.now() - t0).toFixed(1)}ms) chunks ${msg.chunksReused}/${msg.chunksTotal} reused, dist=[${msg.minDist.toFixed(1)}, ${msg.maxDist.toFixed(1)}]`);
        };
